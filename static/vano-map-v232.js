...
let markerHeadingVisual=0,markerHeadingReady=false,lastPhysicalHeading=null,lastPhysicalHeadingAt=0;
...
const routeSwitchCooldown=new Map();const ROUTE_SUGGEST_MIN_S=180,ROUTE_AUTO_MIN_S=180,ROUTE_SWITCH_COOLDOWN_MS=8*60*1000;
...
function makeUserMarker(){
  // One live puck only. TWA/Chrome may restore old DOM after bfcache/session restore.
  document.querySelectorAll('.vano-user-puck-v232,.vano-user-puck-v230,.user-marker.vano-uploaded-user').forEach(node=>{
    if(node!==userMarkerElement){try{node.remove()}catch{}}
  });
  const el=document.createElement('div');
  el.className='vano-user-puck-v232';
  el.setAttribute('aria-hidden','true');
  el.innerHTML=`<span class="vano-puck-accuracy"></span>
  <svg class="vano-puck-inline" viewBox="0 0 80 80" aria-hidden="true" focusable="false">
    <defs>
      <linearGradient id="vanoPuckFace232" x1="40" y1="10" x2="40" y2="62" gradientUnits="userSpaceOnUse">
        <stop offset="0" stop-color="#FFB887"/>
        <stop offset="0.52" stop-color="#F58A4F"/>
        <stop offset="1" stop-color="#D96532"/>
      </linearGradient>
      <filter id="vanoPuckShadow232" x="-40%" y="-30%" width="180%" height="190%">
        <feDropShadow dx="0" dy="5" stdDeviation="4" flood-color="#151922" flood-opacity=".24"/>
      </filter>
    </defs>
    <ellipse cx="40" cy="61.5" rx="16" ry="7" fill="#161A22" opacity=".10"/>
    <path d="M40 6.5 66 66 40 54.6 14 66Z" fill="#FFFFFF" stroke="#D8DDE4" stroke-width="2.3" stroke-linejoin="round" filter="url(#vanoPuckShadow232)"/>
    <path d="M40 12 59.5 58.2 40 49.6 20.5 58.2Z" fill="url(#vanoPuckFace232)" stroke="#C85B2C" stroke-width="1.5" stroke-linejoin="round"/>
    <path d="M40 17 47.5 35.2 40 31.8 32.5 35.2Z" fill="#FFFFFF" opacity=".20"/>
  </svg>`;
  userMarkerElement=el;markerHeadingReady=false;markerHeadingVisual=0;return el
}
function normalizeHeading(v){if(v===null||v===undefined||v==='')return null;const n=Number(v);return Number.isFinite(n)?((n%360)+360)%360:null}
function resolvePhysicalPuckHeading(p){
  if(!p)return Number.isFinite(lastPhysicalHeading)?lastPhysicalHeading:null;
  const speed=Math.max(0,Number.isFinite(+(p._rawSpeed??p.speed))?+(p._rawSpeed??p.speed):0),rawMove=Math.max(0,Number.isFinite(+p._rawMove)?+p._rawMove:0),gps=normalizeHeading(p._rawHeading??p.heading),motion=normalizeHeading(p._motionHeading),route=normalizeHeading(p._routeHeading??(activeNav?.classList.contains('show')?routeAlignedPuckBearing(p):null));
  let heading=null;
  if(speed>=2&&gps!=null)heading=gps;
  else if(rawMove>=4&&motion!=null)heading=motion;
  else if(speed>=.8&&motion!=null)heading=motion;
  else if(route!=null)heading=route;
  else heading=gps??motion??(Number.isFinite(lastPhysicalHeading)?lastPhysicalHeading:null);
  if(speed>=1.5&&gps!=null&&motion!=null){const d=bearingDelta(gps,motion);heading=d<45?blendBearing(gps,motion,.28):(rawMove>=5?motion:gps)}
  // Route bearing is only a stabilizer when the vehicle is already aligned with it.
  // It must never pull a real turn/U-turn back toward the old route.
  if(speed>.8&&route!=null&&heading!=null&&bearingDelta(heading,route)<34)heading=blendBearing(heading,route,.14);
  if(speed<.45&&rawMove<2&&Number.isFinite(lastPhysicalHeading))heading=lastPhysicalHeading;
  heading=normalizeHeading(heading);
  if(heading!=null){lastPhysicalHeading=heading;lastPhysicalHeadingAt=Date.now()}
  return heading
}
function updateMarkerHeading(absoluteHeading,{instant=false}={}){
  let heading=normalizeHeading(absoluteHeading);if(heading==null)heading=Number.isFinite(lastMarkerHeading)?normalizeHeading(lastMarkerHeading):null;if(heading==null)return;
  lastMarkerHeading=heading;
  if(!markerHeadingReady||instant){markerHeadingVisual=heading;markerHeadingReady=true}
  else{const d=bearingDelta(markerHeadingVisual,heading),alpha=d>135?.96:d>80?.88:d>45?.76:d>20?.58:d>8?.42:.28;markerHeadingVisual=blendBearing(markerHeadingVisual,heading,alpha)}
...
function visualPuckLngLat(p){
  ...
  if(!navOn||p._routeSnapped===false||p._mapMatchReleased)return[+p.lon,+p.lat];
...
function paintUserMarker(p){
  ...
  const navOn=!!activeNav?.classList.contains('show')&&!!selectedRoute,heading=resolvePhysicalPuckHeading(p);
  if(!userMarker){
...
function stabilizeActiveNavPosition(filtered,raw,match=null){
  ...
  const m=match||nearestProgress(filtered),acc=Number.isFinite(+raw?.accuracy)?Math.max(1,+raw.accuracy):50,speed=Number.isFinite(+raw?.speed)?Math.max(0,+raw.speed):0,snapLimit=Math.max(18,Math.min(58,acc*1.22)),prevRaw=navLastRawPosition;
  const rawMove=prevRaw?hav([prevRaw.lon,prevRaw.lat],[raw.lon,raw.lat]):Infinity,motionBearing=prevRaw&&rawMove>3.5?bearingBetween([prevRaw.lon,prevRaw.lat],[raw.lon,raw.lat]):null;
  navLastRawPosition={lat:raw.lat,lon:raw.lon,accuracy:acc,speed,heading:raw.heading};
  let along=Math.max(0,+m?.distanceAlong||0),routeHeading=Number.isFinite(along)?routeBearingAtDistance(along,Math.max(24,Math.min(64,28+speed*1.8))):null;
  const headingMeta={_motionHeading:Number.isFinite(motionBearing)?motionBearing:null,_rawHeading:normalizeHeading(raw?.heading),_rawMove:Number.isFinite(rawMove)?rawMove:0,_rawSpeed:speed,_routeHeading:Number.isFinite(routeHeading)?routeHeading:null};
  if(!Number.isFinite(m.offRoute)||m.offRoute>snapLimit){mapMatchMismatchHits=Math.min(6,mapMatchMismatchHits+1);if(mapMatchMismatchHits>=3)mapMatchReleaseUntil=Date.now()+6500;return{...filtered,...headingMeta,_rawOffRoute:m.offRoute,_routeSnapped:false}}
  const headingCandidate=speed>1.6&&Number.isFinite(+raw.heading)?+raw.heading:motionBearing,mismatch=Number.isFinite(routeHeading)&&Number.isFinite(headingCandidate)?bearingDelta(routeHeading,headingCandidate):0;
  ...
  if(Date.now()<mapMatchReleaseUntil)return{...filtered,...headingMeta,_rawOffRoute:m.offRoute,_routeSnapped:false,_mapMatchReleased:true};
  ...
  if(navLastAlong>0){
    ...
    // Prevent a noisy fix from snapping to a later parallel/loop segment.
    const maxForwardJump=Math.max(85,Math.min(260,65+(Number.isFinite(rawMove)?rawMove:0)*7+speed*2.2));
    if(moving&&along>navLastAlong+maxForwardJump)along=navLastAlong+maxForwardJump;
  }
  routeHeading=routeBearingAtDistance(along,Math.max(24,Math.min(64,28+speed*1.8)));headingMeta._routeHeading=Number.isFinite(routeHeading)?routeHeading:null;
  const xy=routePointAtDistance(along);if(!xy)return{...filtered,...headingMeta};
  let strength=moving?(acc<=28?.99:.94):1;if(mismatch>55&&moving)strength=Math.min(strength,.78);if(mismatch>72&&moving)strength=Math.min(strength,.58);
  return{...filtered,...headingMeta,lon:filtered.lon+(xy[0]-filtered.lon)*strength,lat:filtered.lat+(xy[1]-filtered.lat)*strength,_rawOffRoute:m.offRoute,_routeSnapped:true,_distanceAlong:along,_moving:moving}
}
...
function startupRouteBearing(m,p){
  const along=Math.max(0,+m?.distanceAlong||0),samples=[18,32,52,78],bearings=[];
  for(const span of samples){const b=routeBearingAtDistance(Math.max(0,along-2),span);if(Number.isFinite(b))bearings.push({b,w:span<=52?3:1})}
  let route=null;if(bearings.length){let x=0,y=0;for(const v of bearings){const r=v.b*Math.PI/180;x+=Math.sin(r)*v.w;y+=Math.cos(r)*v.w}route=(Math.atan2(x,y)*180/Math.PI+360)%360}
  const physical=normalizeHeading(p?._rawHeading??p?._motionHeading??p?.heading??lastPhysicalHeading),speed=Math.max(0,+(p?._rawSpeed??p?.speed)||0),acc=Math.max(0,+p?.accuracy||0);
  if(physical!=null&&speed>1.7&&acc<70){if(route==null)return physical;const d=bearingDelta(route,physical);if(d>65)return physical;return blendBearing(route,physical,d>32?.28:.18)}
...
function headingLeadPoint(p,heading,meters){
  if(!p||!Number.isFinite(+p.lat)||!Number.isFinite(+p.lon)||!Number.isFinite(+heading))return null;
  const r=+heading*Math.PI/180,latScale=110540,lonScale=111320*Math.max(.2,Math.cos(+p.lat*Math.PI/180));
  return[+p.lon+(Math.sin(r)*meters)/lonScale,+p.lat+(Math.cos(r)*meters)/latScale]
}
function buildCameraTarget(ctx,ts){
  const p=ctx.p,m=cameraDynamicProgress(ctx,ts),speed=Math.max(0,+(p._rawSpeed??p.speed)||0),step=currentStep(m.distanceAlong),turn=upcomingTurn(m,speed),junction=cameraJunctionContext(m,step,turn),signal=roadControlAhead('traffic_signal',m.distanceAlong,70),nearlyStopped=speed<.45,stoppedAtSignal=!!signal&&speed<.9,arrivalApproach=m.remaining<420&&m.progress>.80;
  const calibrating=ts<cameraCalibrationUntil,bearingSpan=calibrating?64:Math.max(34,Math.min(86,42+speed*1.9)),routeBearing=routeBearingAtDistance(Math.max(0,m.distanceAlong-(calibrating?1:4)),bearingSpan),physicalHeading=resolvePhysicalPuckHeading(p),physicalReliable=physicalHeading!=null&&speed>1.15&&(+p.accuracy||0)<85,offRouteVisual=p._routeSnapped===false||!!p._mapMatchReleased;
  let rawTarget=Number.isFinite(routeBearing)?routeBearing:(physicalHeading??(Number.isFinite(lastCameraBearing)?lastCameraBearing:map.getBearing()));
  if(physicalReliable){
    const d=Number.isFinite(routeBearing)?bearingDelta(routeBearing,physicalHeading):180;
    if(offRouteVisual||d>58)rawTarget=physicalHeading;
    else rawTarget=blendBearing(rawTarget,physicalHeading,d>30?.36:.22);
  }
  const startupLocked=Number.isFinite(navStartupBearing)&&ts<navStartupBearingUntil;
  if(startupLocked&&!offRouteVisual){const d=physicalReliable?bearingDelta(navStartupBearing,physicalHeading):0;if(physicalReliable&&d>65)rawTarget=physicalHeading;else{const release=Math.max(0,Math.min(1,1-(navStartupBearingUntil-ts)/2300));rawTarget=blendBearing(navStartupBearing,rawTarget,.18+release*.46)}}
  let zoom,pitch,bearing,center=[+p.lon,+p.lat],padding={top:0,bottom:0,left:0,right:0};const currentXY=!offRouteVisual?routePointAtDistance(m.distanceAlong):null;if(currentXY)center=currentXY;
  const vw=Math.max(320,window.innerWidth||320),vh=Math.max(480,window.innerHeight||480),startupRemain=Math.max(0,(+navCameraStartUntil||0)-ts),startupRatio=Math.min(1,startupRemain/4200),startupBoost=isMotorizedProfile()?(.25*startupRatio):(.18*startupRatio);
  let look=calibrating?NAV_CAMERA_HOME.lookAhead:cameraLookAhead(speed,m);if(!calibrating){if(junction.decision&&!offRouteVisual)look*=junction.large?.84:.92;if(junction.roundabout&&!offRouteVisual)look*=.80;if(arrivalApproach)look*=.70;look=Math.max(isMotorizedProfile()?72:24,Math.min(isMotorizedProfile()?205:54,look));}
  const centerLead=navCameraMode==='top'?.30:(calibrating?NAV_CAMERA_HOME.centerLead:(junction.large&&!offRouteVisual?.58:.68));
  const target=offRouteVisual&&physicalHeading!=null?headingLeadPoint(p,physicalHeading,look*centerLead):routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*centerLead));setDriveCameraMood(speed,junction,turn);
  if(navCameraMode==='top'){
    zoom=(isMotorizedProfile()?16.72:17.10)-Math.min(.34,speed*.013)+startupBoost*.45;pitch=0;bearing=0;if(target&&speed>4)center=target;
  }else{
    if(target)center=target;
    const highSpeedOpen=Math.min(.36,speed*.0105),junctionOpen=!offRouteVisual?(junction.roundabout?.30:(junction.large?.22:0)):0,arrivalT=arrivalApproach?Math.max(0,Math.min(1,(420-m.remaining)/420)):0;
    zoom=calibrating?NAV_CAMERA_HOME.zoom:17.43-highSpeedOpen-junctionOpen+startupBoost*.14+arrivalT*.12;
    pitch=calibrating?NAV_CAMERA_HOME.pitch:(offRouteVisual?52:junction.roundabout?42:junction.large?46:54-arrivalT*8);
    ...
    const bottomBase=calibrating?NAV_CAMERA_HOME.bottomRatio:(junction.large&&!offRouteVisual?.29:.31),topBase=calibrating?NAV_CAMERA_HOME.topRatio:.055;
    padding={top:Math.round(Math.min(88,Math.max(28,vh*topBase))),bottom:Math.round(Math.min(360,Math.max(178,vh*bottomBase))+SAFE_AREA_BOTTOM),left:Math.round(Math.min(52,Math.max(10,vw*.028))),right:Math.round(Math.min(34,Math.max(8,vw*.016)))};
  }
  if((p.accuracy||0)>70)zoom=Math.min(zoom,16.55+startupBoost*.14);if(isMotorizedProfile())zoom=Math.max(15.90,Math.min(17.50,zoom));
  return{center,zoom,pitch,bearing,padding,rawTarget,stopped:nearlyStopped||stoppedAtSignal,junction,instant:!!ctx.instant,arrivalApproach,speed,nearlyStopped,previewing:!!previewing,offRouteVisual}
}
function primeNavigationCamera(p,m){
  if(!map||!p||!m)return;navCameraMode='perspective';navExperienceMode='immersive';const ts=performance.now(),locked=startupRouteBearing(m,p);markCameraIntent('launch',1080);if(Number.isFinite(locked)){navStartupBearing=locked;navStartupBearingUntil=ts+2400;lastCameraBearing=locked}
...
function cameraMotionProfile(target,ts,dt){
  const calibrating=ts<cameraCalibrationUntil,intent=activeCameraIntent(ts),speed=Math.max(0,+target?.speed||0),top=navCameraMode==='top'||intent==='toggle-2d',turning=!!target?.junction?.decision||!!target?.junction?.large||!!target?.junction?.roundabout,stopped=!!target?.stopped||!!target?.nearlyStopped,offRoute=!!target?.offRouteVisual;
  if(calibrating||['launch','recenter','follow','reroute','toggle-3d'].includes(intent))return{label:'settle',posTau:105,viewTau:132,bearingTau:148,centerEps:.07,zoomEps:.0035,bearingEps:.16};
  if(top)return{label:'top',posTau:130,viewTau:155,bearingTau:180,centerEps:.08,zoomEps:.004,bearingEps:.20};
  if(stopped)return{label:'stopped',posTau:230,viewTau:275,bearingTau:620,centerEps:.06,zoomEps:.0035,bearingEps:.16};
  if(offRoute)return{label:'off-route',posTau:118,viewTau:145,bearingTau:132,centerEps:.07,zoomEps:.0038,bearingEps:.16};
  if(turning)return{label:'turn',posTau:132,viewTau:158,bearingTau:168,centerEps:.075,zoomEps:.004,bearingEps:.18};
  if(speed>14)return{label:'fast',posTau:145,viewTau:180,bearingTau:205,centerEps:.09,zoomEps:.0045,bearingEps:.21};
  return{label:'cruise',posTau:155,viewTau:190,bearingTau:220,centerEps:.085,zoomEps:.0044,bearingEps:.22};
}
function cameraMotionTick(ts){
  ...
  internalCameraMoveUntil=performance.now()+120;try{map.jumpTo({center:cameraVisualState.center,zoom:cameraVisualState.zoom,pitch:cameraVisualState.pitch,bearing:cameraVisualState.bearing,padding:cameraVisualState.padding,retainPadding:false})}catch(e){console.debug('[VANO MAPS:camera-motion]',e)};
...
function routeFlowDelay(){if(document.visibilityState==='hidden'||searchInteractionActive||(!selectedRoute&&!activeNav?.classList.contains('show')))return 1400;if(activeNav?.classList.contains('show'))return performanceTier==='eco'?1800:1200;if(performanceTier==='eco')return 1000;if(performanceTier==='normal')return 520;return 360}
...
function calibrateNavigation(){
  ...
  const now=performance.now();followMode=true;mapFollowMode=true;navCameraMode='perspective';markCameraIntent('recenter',1250);cameraCalibrationStartedAt=now;cameraCalibrationUntil=now+980;navStartupBearing=null;navStartupBearingUntil=0;
  const m=nearestProgress(lastNavPosition),routeHome=routeBearingAtDistance(Math.max(0,(+m.distanceAlong||0)-1),58),physical=resolvePhysicalPuckHeading(lastNavPosition),speed=Math.max(0,+(lastNavPosition._rawSpeed??lastNavPosition.speed)||0),offRoute=lastNavPosition._routeSnapped===false||!!lastNavPosition._mapMatchReleased;
  let home=Number.isFinite(routeHome)?routeHome:physical;if(physical!=null&&speed>1.1&&(offRoute||!Number.isFinite(routeHome)||bearingDelta(routeHome,physical)>52))home=physical;if(Number.isFinite(home))lastCameraBearing=home;
...
async function performNavReroute(position,{manual=false}={}){
  ...
  const previousRouteKey=routeClientKey(selectedRoute);rerouting=true;lastRerouteAt=Date.now();productTelemetry('route',manual?'reroute-manual':'reroute-auto','navigation');const notice=$('rerouteNotice'),btn=$('navRecalculateBtn');notice?.classList.add('show');btn?.classList.add('recalculating');if(notice)notice.querySelector('span:last-child').textContent=manual?'Recalculando…':'Ajustando rota…';
  try{
    if(manual)routeResponseCache.clear();
    const physical=resolvePhysicalPuckHeading(p),start={lat:+p.lat,lon:+p.lon,label:'Minha localização',is_gps:true,is_reroute:true,heading:physical??(Number.isFinite(+p.heading)?+p.heading:null),speed:Number.isFinite(+(p._rawSpeed??p.speed))?Math.max(0,+(p._rawSpeed??p.speed)):null,accuracy:Number.isFinite(+p.accuracy)?+p.accuracy:null};
    await fetchRoutes(start,true);const nextRouteKey=routeClientKey(selectedRoute);if(!manual&&previousRouteKey&&nextRouteKey===previousRouteKey)productTelemetry('route','reroute-same-corridor','navigation');
    origin=start;resetOffRouteTracker();resetMapMatchHysteresis();navSuppressedRouteKeys.clear();navAlternativeHitKey='';navAlternativeHitCount=0;buildMetrics();navLastAlong=0;navLastPaintAlong=0;navLastRawPosition=null;lastNavRoutePaintAt=0;
    const m=nearestProgress(p),step=currentStep(m.distanceAlong);navLastAlong=Math.max(0,m.distanceAlong);setNavigationRouteFocus(true);setNavRouteData(navLastAlong);refreshNavigationAlternatives(true);updateRoadLayer();$('nextStreet').textContent=stepStreet(step);$('nextInstruction').textContent=maneuverLabel(step);updateNavSummary(selectedRoute.duration||0,m.remaining,step);markCameraIntent('reroute',1100);scheduleCamera(p,false,m);showToast(manual?'Rota recalculada.':'Rota ajustada ao seu trajeto.');
  }catch(e){if(e?.name!=='AbortError'){lastRerouteAt=Date.now()-30000;showToast(e?.message||'Não foi possível recalcular. Mantendo a rota atual.')}}finally{rerouting=false;notice?.classList.remove('show');btn?.classList.remove('recalculating')}
}
...
function updateNavigation(g,simulated=false){
  ...
  if(Math.abs(navLastAlong-navLastPaintAlong)>4||now-lastNavRoutePaintAt>650){lastNavRoutePaintAt=now;navLastPaintAlong=navLastAlong;setNavRouteData(navLastAlong)}if(now-lastProgressPaintAt>280){lastProgressPaintAt=now;updateTurnHighlight(m.distanceAlong,step)}
...
function syncNavigationHudGeometry(){
  ...
  if(!app||!activeNav?.classList.contains('show'))return;
  const apply=()=>{const nr=activeNav.getBoundingClientRect();if(!nr.width||!nr.height)return;app.style.setProperty('--vano-nav-live-dock-h',`${Math.max(82,Math.ceil(nr.height))}px`);};
  [street,controls,speedo].forEach(el=>{if(!el)return;for(const prop of ['left','right','top','bottom','transform'])el.style.removeProperty(prop)});
  apply();
  if(typeof ResizeObserver!=='undefined'&&!activeNav.__vanoHudResizeObserver){const ro=new ResizeObserver(()=>requestAnimationFrame(apply));ro.observe(activeNav);activeNav.__vanoHudResizeObserver=ro}
}
