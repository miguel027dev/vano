...
function refreshNavigationAlternatives(force=false){
  if(!activeNav?.classList.contains('show'))return;const now=performance.now();if(!force&&now-navAlternativeLastPaintAt<650)return;navAlternativeLastPaintAt=now;const list=navigationAlternativeRoutes();map?.getSource('route-alternatives')?.setData(navAlternativeFC());
...
function buildCameraTarget(ctx,ts){
  ...
  if((p.accuracy||0)>70)zoom=Math.min(zoom,16.58+startupBoost*.16);if(isMotorizedProfile())zoom=Math.max(15.84,Math.min(17.58,zoom));return{center,zoom,pitch,bearing,padding,rawTarget,stopped:nearlyStopped||stoppedAtSignal,junction,instant:!!ctx.instant}
}
function primeNavigationCamera(p,m){
  if(!map||!p||!m)return;navCameraMode='perspective';navExperienceMode='immersive';const ts=performance.now(),locked=startupRouteBearing(m,p);if(Number.isFinite(locked)){navStartupBearing=locked;navStartupBearingUntil=ts+2800;lastCameraBearing=locked;lastRoutePuckBearing=locked}
...
function cameraMotionTick(ts){
  ...
  if(!cameraVisualState){const c=map.getCenter();cameraVisualState={center:[c.lng,c.lat],zoom:map.getZoom(),pitch:map.getPitch(),bearing:map.getBearing(),padding:{top:0,bottom:0,left:0,right:0}}}
  const calibrating=ts<cameraCalibrationUntil,fast=target.instant&&ts-(cameraTargetContext.updatedAt||ts)<420,calibrationT=calibrating?Math.max(0,Math.min(1,(ts-cameraCalibrationStartedAt)/1180)):1,calibrationEase=calibrationT<.5?4*calibrationT*calibrationT*calibrationT:1-Math.pow(-2*calibrationT+2,3)/2,posTau=calibrating?Math.max(92,260-150*calibrationEase):(fast?78:(target.stopped?320:165)),viewTau=calibrating?Math.max(105,310-170*calibrationEase):(fast?105:(target.stopped?380:220)),bearingTau=calibrating?Math.max(120,360-185*calibrationEase):(fast?105:(target.stopped?680:(target.junction?.large?320:270))),ap=1-Math.exp(-dt/posTau),av=1-Math.exp(-dt/viewTau),ab=1-Math.exp(-dt/bearingTau);
...
function setNavigationExperience(mode,{recenter=true,announce=true}={}){
  const immersive=mode==='immersive'||mode==='perspective'||mode==='3d';navExperienceMode=immersive?'immersive':'normal';navCameraMode=immersive?'perspective':'top';followMode=true;lastCameraBearing=immersive?lastCameraBearing:0;syncNavCameraButton();syncFollowButton();syncImmersiveButton();if(recenter&&lastNavPosition)scheduleCamera(lastNavPosition,true,nearestProgress(lastNavPosition));if(announce)showToast(immersive?'Visualização 3D ativada.':'Visualização 2D ativada.')
}
...
function setNavigationFollow(enabled,recenter=true){followMode=!!enabled;if(!followMode)stopCameraMotion();syncFollowButton();document.body.classList.toggle('nav-map-free',!followMode&&!!activeNav?.classList.contains('show'));if(followMode&&recenter&&lastNavPosition)scheduleCamera(lastNavPosition,true,nearestProgress(lastNavPosition))}
...
function calibrateNavigation(){
  ...
  const now=performance.now();followMode=true;mapFollowMode=true;navCameraMode='perspective';cameraCalibrationStartedAt=now;cameraCalibrationUntil=now+1180;navStartupBearing=null;navStartupBearingUntil=0;
...
async function checkEventDisruption(force=false,{pretrip=false}={}){
  ...
  try{
    ...
    if(pretrip&&!navActive){
      if(d.auto_apply&&!manualRouteSelection){selectedRoute=route;eventStickyUntil=Date.now()+12*60*1000;eventStickyName=eventName;renderRoute();showToast(`Rota ajustada para contornar movimento em ${eventName}.`)}else{renderRouteVariants();updateTrafficRadar(true)}
      return;
    }
...
function toggleNavigationCamera(){
  const to3D=navCameraMode==='top';navCameraMode=to3D?'perspective':'top';navExperienceMode=to3D?'immersive':'normal';followMode=true;if(!to3D)lastCameraBearing=0;stopCameraMotion();syncNavCameraButton();syncFollowButton();syncImmersiveButton();haptic(6);showToast(to3D?'Visualização 3D.':'Visualização 2D.');
  if(lastNavPosition&&selectedRoute){scheduleCamera(lastNavPosition,true,nearestProgress(lastNavPosition));return}
  const center=userLocation?[userLocation.lon,userLocation.lat]:undefined;try{map?.stop?.();map?.easeTo?.({...(center?{center}:{}),pitch:to3D?58:0,bearing:to3D?map.getBearing():0,zoom:to3D?Math.max(16.8,map.getZoom()):Math.max(16.2,map.getZoom()),padding:{top:0,bottom:0,left:0,right:0},retainPadding:false,duration:360,essential:true,easing:t=>1-Math.pow(1-t,3)})}catch(e){console.debug('[VANO MAPS:camera-toggle]',e)}
}
...
function stopRoutePreview(restore=true){if(previewFrame)cancelAnimationFrame(previewFrame);previewFrame=null;previewing=false;stopCameraMotion();setPreviewUi(false);if(restore&&selectedRoute){followMode=false;drawRoute();if(userLocation)updateUserMarker({...userLocation,accuracy:0,heading:null,speed:0})}}
