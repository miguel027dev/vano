(()=>{
  ...
  const legacyHashes={'#appSettings':'#preferences','#places':'#destinations','#family':'#security'};
  if(location.hash){const resolved=legacyHashes[location.hash]||location.hash;const target=document.querySelector(resolved);if(target){if(resolved!==location.hash)history.replaceState(null,'',resolved);setActiveNav(target.id);setTimeout(()=>target.scrollIntoView({behavior:'auto',block:'start'}),30);}}
