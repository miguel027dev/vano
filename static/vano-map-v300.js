...
function animateCalibrationButton(){
  ...
  cameraCalibrationTimer=setTimeout(()=>{ids.forEach(id=>$(id)?.classList.remove('calibrating'));cameraCalibrationTimer=null},1040);
}
function calibrateNavigation(){
  if(!lastNavPosition||!selectedRoute){locateUser(true);return}
  const now=performance.now();followMode=true;mapFollowMode=true;navCameraMode='perspective';setNavCameraState(NAV_CAMERA_STATES.RECENTERING);markCameraIntent('recenter',1420);cameraCalibrationStartedAt=now;cameraCalibrationUntil=now+1040;navStartupBearing=null;navStartupBearingUntil=0;
