...
function bindRouteMoreDetails(){
  const details=$('routeMoreDetails');if(!details)return;const summary=details.querySelector(':scope>summary'),content=details.querySelector(':scope>.vano-route-more-content');
  const sync=()=>{const open=!!details.open;details.classList.toggle('is-open',open);summary?.setAttribute('aria-expanded',String(open));content?.setAttribute('aria-hidden',String(!open));requestAnimationFrame(()=>{try{syncFloatingLocate()}catch{}try{map?.resize?.()}catch{};if(open){const top=Math.max(0,details.offsetTop-10);try{routeState?.scrollTo?.({top,behavior:REDUCED_MOTION?'auto':'smooth'})}catch{if(routeState)routeState.scrollTop=top}}})};
  summary?.setAttribute('role','button');summary?.addEventListener('pointerdown',e=>e.stopPropagation());summary?.addEventListener('click',e=>e.stopPropagation());details.addEventListener('toggle',sync);sync();
}
