...
const VANO_ROUTE='#FF9500', VANO_ROUTE_LIGHT='#FFC266', VANO_ROUTE_DARK=VANO_ROUTE;
...
bindClick('guestTrialPill',()=>guestRoutesRemaining<=0?showGuestLimit():showToast(`${guestRoutesRemaining} de ${GUEST_ROUTE_LIMIT} rotas grátis restantes.`),true);bindClick('guestLimitBackdrop',hideGuestLimit);bindClick('guestLimitLater',hideGuestLimit);renderGuestTrial(guestRoutesRemaining);
...
const NAV_AUDIO_PRIORITY=Object.freeze({navigation:1,radar:2,maneuver:3,reroute:4,safety:5,arrival:6});
let navAudioPriority=0,navAudioPriorityUntil=0;const navAudioCooldown=new Map();
function navAudioKind(text,key=''){const s=normalizeVanoText(`${key} ${text}`);if(s.includes('chegou')||s.includes('arrival'))return'arrival';if(/assalto|perigo|seguranca|segurança|safety/.test(s))return'safety';if(/recalcul|rerout|ajustando rota/.test(s))return'reroute';if(/radar|velocidade/.test(s))return'radar';if(/vire|curva|retorno|rotatoria|rotatória|saida|saída|mantenha/.test(s))return'maneuver';return'navigation'}
function canPlayNavAudio(kind,key){const now=Date.now(),priority=NAV_AUDIO_PRIORITY[kind]||1,cooldown=kind==='navigation'?1800:kind==='maneuver'?2600:kind==='radar'?90000:kind==='safety'?12000:kind==='reroute'?8000:45000,last=navAudioCooldown.get(`${kind}:${key}`)||0;if(now-last<cooldown)return false;if(now<navAudioPriorityUntil&&priority<navAudioPriority)return false;navAudioCooldown.set(`${kind}:${key}`,now);navAudioPriority=priority;navAudioPriorityUntil=now+(kind==='arrival'?6500:kind==='safety'?5200:kind==='maneuver'?4200:2800);return true}

let arrivalPresented=false,tripStartRouteDistance=0,tripStartRouteEta=0,tripRerouteCount=0,tripStartMode='',tripStartSafetyScore=null;
const NAV_CAMERA_HOME={zoom:17.18,pitch:52,lookAhead:126,centerLead:.62,bottomRatio:.285,topRatio:.045};
...
const ALERT_DOM_MARKER_MIN_ZOOM=13.6,ALERT_DOM_MARKER_MAX=45;
...
const NAV_OFF_ROUTE_MIN_TRAVEL_M=70,NAV_OFF_ROUTE_MIN_TIME_MS=5500,NAV_OFF_ROUTE_MAX_ACCURACY_M=95;
let offRouteStartedAt=0,offRouteTravelM=0,offRouteStartRaw=null,offRouteLastRaw=null,offRoutePeakM=0,offRouteClearHits=0;
function resetOffRouteTracker(){offRouteStartedAt=0;offRouteTravelM=0;offRouteStartRaw=null;offRouteLastRaw=null;offRoutePeakM=0;offRouteClearHits=0;offRouteCount=0}
function offRouteThreshold(raw){const acc=Math.max(4,Number.isFinite(+raw?.accuracy)?+raw.accuracy:35);return Math.max(48,Math.min(105,acc*1.48))}
function updateOffRouteTracker(raw,offRoute){
  if(!raw||!Number.isFinite(+offRoute)||!Number.isFinite(+raw.lat)||!Number.isFinite(+raw.lon))return false;
  const acc=Math.max(1,Number.isFinite(+raw.accuracy)?+raw.accuracy:999),threshold=offRouteThreshold(raw),now=Date.now();
  // Poor GPS must not erase an already established deviation. We simply pause accumulation.
  if(acc>NAV_OFF_ROUTE_MAX_ACCURACY_M){offRouteClearHits=0;return false}
  const confident=offRoute>threshold;
  if(!confident){
    if(!offRouteStartedAt)return false;
    // Require several clearly-on-route fixes before clearing the deviation state.
    if(offRoute<threshold*.58)offRouteClearHits++;else offRouteClearHits=Math.max(0,offRouteClearHits-1);
    if(offRouteClearHits>=3)resetOffRouteTracker();
    return false
  }
  offRouteClearHits=0;const point={lat:+raw.lat,lon:+raw.lon,accuracy:acc};
  ...
  if(offRouteLastRaw){const step=hav([offRouteLastRaw.lon,offRouteLastRaw.lat],[point.lon,point.lat]);const plausible=Math.max(80,Math.min(210,Math.max(acc,offRouteLastRaw.accuracy||acc)*3));if(Number.isFinite(step)&&step<=plausible)offRouteTravelM+=step}
...
const LOCAL_RECENT_DEST_KEY='vano.recentDestinations.v310',LOCAL_RECENT_DEST_MAX=12;
function readLocalRecentDestinations(){try{const raw=JSON.parse(localStorage.getItem(LOCAL_RECENT_DEST_KEY)||'[]');return Array.isArray(raw)?raw.filter(x=>Number.isFinite(+x?.lat)&&Number.isFinite(+x?.lon)&&x?.label).slice(0,LOCAL_RECENT_DEST_MAX):[]}catch{return[]}}
...
function saveMapPrefs(){const status=$('prefsSaveStatus');try{localStorage.setItem(MAP_PREF_KEY,JSON.stringify(mapPrefs));if(status)status.textContent='Preferências salvas neste aparelho.';return true}catch{if(status)status.textContent='Preferências aplicadas. O navegador não permitiu salvar para a próxima visita.';return false}}
...
function smartBalanceFromPrefs(){const safety=Math.max(42,Math.min(90,+sparkPrefs.safety||68));return Math.max(0,Math.min(100,Math.round((safety-45)/.45)))}
function applySmartBalance(value,{recalculate=true}={}){const v=Math.max(0,Math.min(100,+value||0));sparkPrefs.safety=Math.round(45+v*.45);sparkPrefs.traffic=Math.round(82-v*.22);saveSparkPrefs();const label=$('smartBalanceLabel'),hint=$('smartBalanceHint'),box=$('smartBalanceBox');if(label)label.textContent=v<35?'Prioriza rapidez':v>70?'Prioriza segurança':'Equilibrado';if(hint)hint.textContent=v<35?'Aceita mais exposição quando isso reduz o ETA de forma relevante.':v>70?'Aceita um pequeno desvio quando ele reduz exposição e instabilidade.':'Equilibra ETA, trânsito, alertas e estabilidade da rota.';box?.classList.toggle('safety-heavy',v>70);box?.classList.toggle('speed-heavy',v<35);routeResponseCache.clear();if(recalculate&&routeMode==='smart'&&destinationConfirmed&&origin&&destination)calculateRoutes(true)}
function syncSmartBalanceUI(){const input=$('smartBalance');if(!input)return;const v=smartBalanceFromPrefs();input.value=String(v);applySmartBalance(v,{recalculate:false});$('smartBalanceBox')?.classList.toggle('active',routeMode==='smart')}
...
function renderRouteSummaryPills(route){const box=$('routeSummaryPills'),count=$('routeChooserCount');if(count)count.textContent=(routes||[]).length>1?`${Math.min(4,(routes||[]).length)} opções comparadas`:'1 rota pronta';if(!box||!route)return;const pills=[];pills.push(`<span class="route-summary-pill emphasis"><i data-lucide="${routeMode==='fastest'?'zap':routeMode==='smart'?'sparkles':'shield-check'}" width="13"></i><b>${routeMode==='fastest'?'Modo ETA':routeMode==='smart'?'Modo Spark':'Modo seguro'}</b></span>`);box.innerHTML=pills.join('');}
function routeModeLabel(){return routeMode==='fastest'?'Mais rápida':routeMode==='smart'?'Spark':'Mais segura'}
...
let accountDrawerReturnFocus=null;
function openAccountDrawer(force=true){
  const drawer=$('accountDrawer'),backdrop=$('accountBackdrop');if(!drawer||!backdrop)return;
  const wasOpen=drawer.classList.contains('show'),show=force===null?!wasOpen:!!force;
  if(show&&!wasOpen){
    accountDrawerReturnFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;
    try{openPrefsDrawer(false)}catch{}try{openSafetyDrawer(false)}catch{}try{toggleVoicePopover(false)}catch{}try{setNavControlDrawer(false)}catch{}try{hideResults()}catch{}
    drawer.style.removeProperty('transform');drawer.style.removeProperty('transition');drawer.scrollTop=0;
  }
  drawer.classList.toggle('show',show);backdrop.classList.toggle('show',show);drawer.setAttribute('aria-hidden',String(!show));backdrop.setAttribute('aria-hidden',String(!show));$('optionsBtn')?.setAttribute('aria-expanded',String(show));document.body.classList.toggle('vano-account-open',show);
  if(show&&!wasOpen){if(window.lucide)lucide.createIcons();requestAnimationFrame(()=>{const focusTarget=drawer.querySelector('.v258-drawer-close,.guest-drawer-close,[data-account-close],a,button');focusTarget?.focus?.({preventScroll:true})})}
  else if(!show&&wasOpen){
    drawer.style.removeProperty('transform');drawer.style.removeProperty('transition');
    requestAnimationFrame(()=>{try{restoreMapGestureHealth()}catch{}try{map?.resize?.()}catch{}try{syncFloatingLocate()}catch{};if(accountDrawerReturnFocus?.isConnected)accountDrawerReturnFocus.focus?.({preventScroll:true});accountDrawerReturnFocus=null})
  }
}
function bindAccountDrawerDrag(){
  const drawer=$('accountDrawer'),grab=drawer?.querySelector('.drawer-grab');if(!drawer||!grab)return;
  let active=false,pointerId=null,startY=0,delta=0,moved=false;
  const reset=()=>{active=false;pointerId=null;drawer.style.removeProperty('transition');drawer.style.removeProperty('transform')};
  grab.style.touchAction='none';
  grab.addEventListener('pointerdown',e=>{if(!drawer.classList.contains('show')||e.button>0)return;active=true;pointerId=e.pointerId;moved=false;delta=0;startY=e.clientY;drawer.style.transition='none';try{grab.setPointerCapture?.(e.pointerId)}catch{}});
  grab.addEventListener('pointermove',e=>{if(!active||e.pointerId!==pointerId)return;delta=Math.max(0,e.clientY-startY);if(delta>4)moved=true;if(!moved)return;e.preventDefault();drawer.style.transform=`translate3d(0,${Math.min(delta,Math.max(110,innerHeight*.42))}px,0)`},{passive:false});
  const finish=e=>{if(!active||(e?.pointerId!=null&&e.pointerId!==pointerId))return;const shouldClose=delta>Math.max(72,Math.min(118,innerHeight*.12));try{grab.releasePointerCapture?.(pointerId)}catch{};reset();if(shouldClose)openAccountDrawer(false)};
  grab.addEventListener('pointerup',finish);grab.addEventListener('pointercancel',finish);grab.addEventListener('lostpointercapture',()=>{if(active)reset()});
  grab.addEventListener('click',e=>{if(moved){e.preventDefault();e.stopPropagation()}});
}
...
function routeSafetyScore(route){return Math.max(0,Math.min(100,Number(route?.safety_conservative_score??route?.safety_score??((+route?.safety_level||0)*20))||0))}
function routeExposurePct(route){const explicit=Number(route?.risk_exposure_pct);if(Number.isFinite(explicit))return Math.max(0,Math.min(100,explicit));const zones=(route?.risk_zones||[]).length,alerts=(route?.nearby_alerts||[]).filter(a=>(+a.severity||0)>=3).length;return Math.max(0,Math.min(100,zones*4+alerts*2))}
function etaBand(seconds,route=selectedRoute){const base=Math.max(0,+seconds||0),traffic=Math.max(0,Math.min(100,+route?.traffic_score||45)),confidence=Math.max(20,Math.min(100,+route?.decision_confidence||+route?.data_confidence||72));const uncertainty=Math.max(45,Math.min(420,base*(.035+(traffic/100)*.045+((100-confidence)/100)*.055)));return{low:Math.max(0,base-uncertainty*.35),high:base+uncertainty}}
function etaBandLabel(seconds,route=selectedRoute){const b=etaBand(seconds,route),lo=Math.max(1,Math.ceil(b.low/60)),hi=Math.max(lo,Math.ceil(b.high/60));return lo===hi?`${hi} min provável`:`${lo}–${hi} min provável`}
function routeTrustExplanation(route){if(!route)return'';const safety=Math.round(routeSafetyScore(route)),exposure=routeExposurePct(route),confidence=Math.round(Math.max(0,Math.min(100,+route.decision_confidence||+route.data_confidence||0))),pieces=[];if(routeMode!=='fastest'&&safety)pieces.push(`segurança ${safety}/100`);if(routeMode!=='fastest'&&Number.isFinite(exposure))pieces.push(`${exposure.toFixed(exposure<10?1:0)}% de exposição estimada`);if(confidence)pieces.push(`confiança ${confidence}%`);return pieces.slice(0,3).join(' · ')}
function nextStepAfter(step){const steps=selectedRoute?.steps||[],idx=Number.isFinite(+step?.stepIndex)?+step.stepIndex:-1;return idx>=0&&idx+1<steps.length?{...steps[idx+1],stepIndex:idx+1}:null}
function updateFollowingManeuver(step){const box=$('navFollowingStep'),b=$('followingInstruction'),em=$('followingDistance');if(!box||!b||!em)return;const next=nextStepAfter(step);if(!next||String(next.type||'').toLowerCase()==='arrive'){box.classList.remove('show');return}const viewportW=Math.max(document.documentElement?.clientWidth||0,window.innerWidth||0),viewportH=Math.max(document.documentElement?.clientHeight||0,window.innerHeight||0),nextLabel=maneuverLabel(next),availableHeight=Math.max(0,viewportH-((($('navManeuverTop')?.getBoundingClientRect().height)||0)+(($('activeNav')?.getBoundingClientRect().height)||0)+120)),crowded=viewportW<390||viewportH<700||availableHeight<150||nextLabel.length>34;if(crowded){box.classList.remove('show');return}const dist=Math.max(0,+step?.remainingInStep||0)+Math.max(0,+next.distance||0);b.textContent=nextLabel;em.textContent=dist<1200?fmtDistance(dist):`em ~${fmtDistance(dist)}`;box.classList.add('show')}
function updateNavContextAlert(m,step){const box=$('navContextAlert'),label=$('navContextLabel'),text=$('navContextText'),distance=$('navContextDistance');if(!box||!label||!text||!distance)return;const risk=nextRiskAhead(m),lanes=step?.lanes||[],laneSoon=isMotorizedProfile()&&lanes.length&&(step?.remainingInStep??9999)<=420;if(risk&&risk.distance<=900){label.textContent=risk.kind==='event'?'MOVIMENTO À FRENTE':risk.kind==='zone'?'ÁREA DE ATENÇÃO':'ALERTA À FRENTE';text.textContent=risk.title;distance.textContent=risk.distance<25?'agora':fmtDistance(risk.distance);box.dataset.kind='risk';box.hidden=false;return}if(laneSoon){const valid=lanes.filter(x=>x.valid).length;label.textContent='FAIXA RECOMENDADA';text.textContent=valid===1?'Posicione-se na faixa indicada':`${valid} faixas válidas para a manobra`;distance.textContent=(+step.remainingInStep||0)<25?'agora':fmtDistance(step.remainingInStep);box.dataset.kind='lane';box.hidden=false;return}box.hidden=true;box.removeAttribute('data-kind')}
function syncConnectivityUI(){const offline=navigator.onLine===false,pill=$('navConnectivityPill');document.body.classList.toggle('vano-offline',offline);if(pill){pill.hidden=!offline;pill.classList.toggle('show',offline)}if(offline&&!offlineSince){offlineSince=Date.now();showToast('Sem internet · a rota atual continua disponível.')}if(!offline&&offlineSince){offlineSince=0;showToast('Conexão restaurada. Atualizando trânsito e alternativas…');if(activeNav?.classList.contains('show')){checkLiveTraffic(true);refreshNavigationAlternatives(true)}}}
function tripElapsedSeconds(){return navStartedAt?Math.max(0,(Date.now()-navStartedAt)/1000):0}
function syncArrivalMetrics(){const time=$('arrivalTripTime'),dist=$('arrivalTripDistance'),mode=$('arrivalTripMode'),reroutes=$('arrivalTripReroutes');if(time)time.textContent=fmtRemaining(tripElapsedSeconds());if(dist)dist.textContent=fmtDistance(gpsDistance>30?gpsDistance:(tripStartRouteDistance||selectedRoute?.distance||0));if(mode)mode.textContent=tripStartMode||routeModeLabel();if(reroutes)reroutes.textContent=String(tripRerouteCount)}
...
function routeAlignedPuckBearing(p){
  ...
  const rawOff=Number.isFinite(+p?._rawOffRoute)?+p._rawOffRoute:null,acc=Math.max(4,+p?.accuracy||35),snapLimit=Math.max(34,Math.min(92,acc*1.55));
  if(p?._routeSnapped===false||(Number.isFinite(rawOff)&&rawOff>snapLimit))return null;
  const along=Number.isFinite(+p?._distanceAlong)?+p._distanceAlong:+nearestProgress(p).distanceAlong||0,speed=Math.max(0,+p?.speed||0),span=Math.max(28,Math.min(82,36+speed*2.2)),raw=routeBearingAtDistance(along,span);
  if(!Number.isFinite(raw))return Number.isFinite(lastRoutePuckBearing)?lastRoutePuckBearing:null;
  if(!Number.isFinite(lastRoutePuckBearing))lastRoutePuckBearing=raw;else lastRoutePuckBearing=blendBearing(lastRoutePuckBearing,raw,speed>=8?.26:speed>=2?.20:.14);
...
function paintUserMarker(p){
  ...
  updateMarkerHeading(heading,{instant:!markerHeadingReady,speed:p.speed,accuracy:p.accuracy});
}
...
function stabilizeActiveNavPosition(filtered,raw,match=null){
  ...
  if(!routeCumulative.length)buildMetrics();const m=match||nearestProgress(filtered),acc=Number.isFinite(+raw?.accuracy)?Math.max(1,+raw.accuracy):50,speed=Number.isFinite(+raw?.speed)?Math.max(0,+raw.speed):0,snapLimit=Math.max(20,Math.min(72,acc*1.35)),prevRaw=navLastRawPosition;
...
function cameraVehicleTuning(){
  if(profile==='motorcycle')return{kind:'motorcycle',lookMin:88,lookMax:372,lookBase:132,lookPerSpeed:7.2,turnLook:.72,sharpLook:.56,decisionDistance:270,largeAngle:34,centerLead:.76,turnLead:.66,largeLead:.60,roundLead:.56,zoomBase:17.34,zoomMin:16.30,zoomMax:17.42,zoomPerSpeed:.0155,zoomSpeedOpen:.54,pitchCruise:53,pitchTurn:48,pitchLarge:44,pitchRound:40,bearingCruiseRate:172,bearingDecisionRate:245,bearingLargeRate:282,bearingRoundRate:315,bearingAlphaSlow:.42,bearingAlphaCruise:.58,bearingAlphaFast:.68,bearingAlphaDecision:.80,bearingAlphaLarge:.86,bearingAlphaRound:.90};
  if(profile==='driving')return{kind:'driving',lookMin:78,lookMax:330,lookBase:118,lookPerSpeed:6.1,turnLook:.76,sharpLook:.62,decisionDistance:225,largeAngle:38,centerLead:.72,turnLead:.68,largeLead:.62,roundLead:.58,zoomBase:17.24,zoomMin:16.35,zoomMax:17.30,zoomPerSpeed:.017,zoomSpeedOpen:.48,pitchCruise:52,pitchTurn:47,pitchLarge:44,pitchRound:41,bearingCruiseRate:135,bearingDecisionRate:190,bearingLargeRate:225,bearingRoundRate:240,bearingAlphaSlow:.34,bearingAlphaCruise:.46,bearingAlphaFast:.54,bearingAlphaDecision:.70,bearingAlphaLarge:.78,bearingAlphaRound:.84};
  return{kind:'other',lookMin:28,lookMax:58,lookBase:34,lookPerSpeed:1.8,turnLook:.78,sharpLook:.66,decisionDistance:120,largeAngle:42,centerLead:.55,turnLead:.48,largeLead:.44,roundLead:.42,zoomBase:17.05,zoomMin:16.55,zoomMax:17.35,zoomPerSpeed:.02,zoomSpeedOpen:.34,pitchCruise:42,pitchTurn:38,pitchLarge:36,pitchRound:34,bearingCruiseRate:105,bearingDecisionRate:145,bearingLargeRate:170,bearingRoundRate:190,bearingAlphaSlow:.30,bearingAlphaCruise:.40,bearingAlphaFast:.46,bearingAlphaDecision:.62,bearingAlphaLarge:.70,bearingAlphaRound:.76}
}
function resetCameraKinematics(){cameraKinematicSpeed=0;cameraKinematicAccel=0;cameraKinematicTs=0;cameraKinematicPhase='steady'}
function cameraKinematics(speed,ts){
  const v=Math.max(0,Math.min(60,+speed||0));if(!cameraKinematicTs){cameraKinematicSpeed=v;cameraKinematicTs=ts;cameraKinematicAccel=0;cameraKinematicPhase=v<.45?'stopped':v<2.2?'crawl':'steady';return{speed:v,accel:0,phase:cameraKinematicPhase}}
  const dt=Math.max(.08,Math.min(1.4,(ts-cameraKinematicTs)/1000));cameraKinematicTs=ts;const rawAccel=Math.max(-7,Math.min(5,(v-cameraKinematicSpeed)/dt)),alpha=profile==='motorcycle'?.32:.22;cameraKinematicAccel+=(rawAccel-cameraKinematicAccel)*alpha;cameraKinematicSpeed+=(v-cameraKinematicSpeed)*(profile==='motorcycle'?.45:.34);
  const a=cameraKinematicAccel;cameraKinematicPhase=v<.45?'stopped':v<2.2?'crawl':a>.72?'accelerating':a<-1.05?'braking':'steady';return{speed:v,accel:a,phase:cameraKinematicPhase}
}
...
function stabilizeCameraTargetBearing(raw,ts,speed=0,junction=null){
  if(!Number.isFinite(+raw))return raw;const normalized=(+raw%360+360)%360,tune=cameraVehicleTuning();
  ...
  let delta=((normalized-cameraTargetBearing+540)%360)-180;const deadband=profile==='motorcycle'?(speed>5?.12:.24):.18;if(Math.abs(delta)<deadband)return cameraTargetBearing;
  const decision=!!junction?.decision,large=!!junction?.large,roundabout=!!junction?.roundabout;
  const rate=roundabout?tune.bearingRoundRate:large?tune.bearingLargeRate:decision?tune.bearingDecisionRate:Math.max(profile==='motorcycle'?92:72,Math.min(tune.bearingCruiseRate,(profile==='motorcycle'?108:82)+Math.max(0,+speed||0)*(profile==='motorcycle'?3.8:2.4))),maxStep=Math.max(profile==='motorcycle'?1.55:1.2,rate*dt/1000);
  delta=Math.max(-maxStep,Math.min(maxStep,delta));
  const alpha=roundabout?tune.bearingAlphaRound:large?tune.bearingAlphaLarge:decision?tune.bearingAlphaDecision:Math.max(0,+speed||0)>14?tune.bearingAlphaFast:Math.max(0,+speed||0)>4?tune.bearingAlphaCruise:tune.bearingAlphaSlow;
  cameraTargetBearing=(cameraTargetBearing+delta*alpha+360)%360;return cameraTargetBearing
}
function navigationHudCameraInsets(){
  const app=$('wsApp'),topCard=$('navManeuverTop'),bottomCard=activeNav;if(!app||!activeNav?.classList.contains('show'))return{top:0,bottom:0,left:0,right:0};
  const ar=app.getBoundingClientRect(),tr=topCard?.getBoundingClientRect(),br=bottomCard?.getBoundingClientRect(),vw=Math.max(320,ar.width||window.innerWidth||320),vh=Math.max(320,ar.height||window.innerHeight||480),landscape=vw>vh;
  if(landscape&&vw>=600)return{top:18,bottom:18+SAFE_AREA_BOTTOM,left:Math.round(Math.min(320,Math.max(205,vw*.27))),right:Math.round(Math.min(82,Math.max(46,vw*.055)))};
  return{top:Math.round(Math.max(18,(tr?.bottom||ar.top)-ar.top+16)),bottom:Math.round(Math.max(90,ar.bottom-(br?.top||ar.bottom)+18+SAFE_AREA_BOTTOM)),left:Math.round(Math.max(12,Math.min(30,vw*.028))),right:Math.round(Math.max(12,Math.min(30,vw*.028)))}
}
function syncManeuverCardDensity(step){const card=$('navManeuverTop');if(!card)return;const instruction=maneuverLabel(step),street=stepStreet(step),vw=Math.max(document.documentElement?.clientWidth||0,window.innerWidth||0),vh=Math.max(document.documentElement?.clientHeight||0,window.innerHeight||0),score=instruction.length+Math.min(38,street.length*.55)+(vw<390?14:vw<430?8:0)+(vh<730?8:0);card.dataset.density=score>64?'tight':score>48?'compact':'normal';card.dataset.maneuverType=String(step?.type||'continue').toLowerCase()}
function buildCameraTarget(ctx,ts){
  const p=ctx.p,m=cameraDynamicProgress(ctx,ts),speed=Math.max(0,+p.speed||0),motion=cameraKinematics(speed,ts),tune=cameraVehicleTuning(),step=currentStep(m.distanceAlong),turn=upcomingTurn(m,speed),junction=cameraJunctionContext(m,step,turn),signal=roadControlAhead('traffic_signal',m.distanceAlong,70),nearlyStopped=speed<.45,stoppedAtSignal=!!signal&&speed<.9,arrivalApproach=m.remaining<420&&m.progress>.80;
  ...
  const offRoute=Number.isFinite(+p?._rawOffRoute)?+p._rawOffRoute:0,bearingSpan=calibrating?72:Math.max(profile==='motorcycle'?38:42,Math.min(profile==='motorcycle'?112:125,(profile==='motorcycle'?46:52)+speed*(profile==='motorcycle'?2.8:3.2))),routeBearing=routeBearingAtDistance(Math.max(0,m.distanceAlong-(calibrating?2:(profile==='motorcycle'?4:6))),bearingSpan),gpsHeading=Number.isFinite(+p.heading)?+p.heading:null;
  ...
  if(offRoute>Math.max(55,(+p.accuracy||35)*1.5)&&gpsHeading!=null)rawTarget=gpsHeading;
  const startupLocked=Number.isFinite(navStartupBearing)&&ts<navStartupBearingUntil;
  if(startupLocked){const release=Math.max(0,Math.min(1,1-(navStartupBearingUntil-ts)/2600));rawTarget=blendBearing(navStartupBearing,rawTarget,release*(profile==='motorcycle'?.34:.28))}
  else if(gpsHeading!=null&&speed>(profile==='motorcycle'?2.5:3.2)&&(p.accuracy||0)<55&&bearingDelta(rawTarget,gpsHeading)<(profile==='motorcycle'?38:32))rawTarget=blendBearing(rawTarget,gpsHeading,profile==='motorcycle'?.18:.14);
  ...
  let look=calibrating?NAV_CAMERA_HOME.lookAhead:cameraLookAhead(speed,m,motion);if(!calibrating){if(junction.decision)look*=junction.large?.88:.94;if(junction.roundabout)look*=profile==='motorcycle'?.82:.76;if(arrivalApproach)look*=.68;look*=profile==='motorcycle'?.94:.88;look=Math.max(tune.lookMin,Math.min(profile==='motorcycle'?285:isMotorizedProfile()?220:54,look));if(startupRatio>.01)look*=.82+.18*(1-startupRatio)}
  let centerLead=navCameraMode==='top'?.34:(calibrating?NAV_CAMERA_HOME.centerLead:(junction.roundabout?tune.roundLead:junction.large?tune.largeLead:junction.decision?tune.turnLead:tune.centerLead));if(motion.phase==='braking')centerLead*=profile==='motorcycle'?.88:.92;if(motion.phase==='crawl')centerLead*=.90;if(nearlyStopped)centerLead*=.82;const target=routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*centerLead));setDriveCameraMood(speed,junction,turn);
  if(navCameraMode==='top'){
    if(target&&speed>3)center=routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*(profile==='motorcycle'?.28:.24)))||center;
    zoom=Math.max(profile==='motorcycle'?16.0:16.05,Math.min(profile==='motorcycle'?17.12:17.05,(profile==='motorcycle'?16.92:16.86)-Math.min(profile==='motorcycle'?.54:.48,speed*(profile==='motorcycle'?.017:.018))));pitch=0;bearing=0;padding=landscape?{top:18,bottom:20+SAFE_AREA_BOTTOM,left:Math.round(Math.min(382,vw*.46)+22),right:68}:{top:26,bottom:72+SAFE_AREA_BOTTOM,left:18,right:18};
  }else{
    if(target)center=target;
    const speedOpen=Math.min(tune.zoomSpeedOpen,speed*tune.zoomPerSpeed),turnOpen=junction.roundabout?(profile==='motorcycle'?.40:.44):junction.large?(profile==='motorcycle'?.31:.34):(junction.decision?(profile==='motorcycle'?.14:.16):0),arrivalClose=arrivalApproach?.14:0,accelOpen=motion.phase==='accelerating'?(profile==='motorcycle'?.07:.04):0,brakeClose=motion.phase==='braking'?(profile==='motorcycle'?.06:.035):0;
    ...
    const bottomBase=calibrating?NAV_CAMERA_HOME.bottomRatio:(profile==='motorcycle'?(junction.large?.285:.305):(junction.large?.30:.33)),topBase=calibrating?NAV_CAMERA_HOME.topRatio:(junction.large?.075:.055);
    const hud=navigationHudCameraInsets();padding=landscape?{top:Math.max(Math.round(Math.max(22,vh*.055)),hud.top),bottom:Math.max(Math.round(Math.max(42,vh*.12)+SAFE_AREA_BOTTOM),hud.bottom),left:Math.max(Math.round(Math.max(210,Math.min(330,vw*.30))),hud.left),right:Math.max(Math.round(Math.max(42,vw*.055)),hud.right)}:{top:Math.max(Math.round(Math.min(150,Math.max(24,vh*topBase))),hud.top),bottom:Math.max(Math.round(Math.min(360,Math.max(profile==='motorcycle'?158:170,vh*bottomBase))+SAFE_AREA_BOTTOM),hud.bottom),left:Math.max(Math.round(Math.min(48,Math.max(10,vw*.026))),hud.left),right:Math.max(Math.round(Math.min(32,Math.max(8,vw*.016))),hud.right)};
  }
  if((p.accuracy||0)>70)zoom=Math.min(zoom,(profile==='motorcycle'?16.48:16.58)+startupBoost*.16);if((p.accuracy||0)>110&&Number.isFinite(lastCameraBearing))bearing=blendBearing(lastCameraBearing,bearing,profile==='motorcycle'?.18:.12);if(isMotorizedProfile())zoom=Math.max(profile==='motorcycle'?16.05:15.84,Math.min(profile==='motorcycle'?17.48:17.58,zoom));if(Number.isFinite(navLastCameraZoom)&&Math.abs(zoom-navLastCameraZoom)<.018)zoom=navLastCameraZoom;return{center,zoom,pitch,bearing,padding,rawTarget,stopped:nearlyStopped||stoppedAtSignal,junction,instant:!!ctx.instant,arrivalApproach,speed,nearlyStopped,previewing:!!previewing,motion}
}
...
function cameraMotionProfile(target,ts,dt){
  const calibrating=ts<cameraCalibrationUntil,moto=profile==='motorcycle';
  if(calibrating){const t=Math.max(0,Math.min(1,(ts-cameraCalibrationStartedAt)/(moto?880:1040))),e=t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;return{label:'calibrating',posTau:Math.max(moto?72:88,(moto?205:255)-(moto?122:150)*e),viewTau:Math.max(moto?82:100,(moto?235:300)-(moto?138:170)*e),bearingTau:Math.max(moto?88:118,(moto?245:350)-(moto?145:185)*e),centerEps:.07,zoomEps:.0035,bearingEps:moto?.13:.16}}
  const speed=Math.max(0,+target?.speed||0),intent=activeCameraIntent(ts),largeTurn=!!target?.junction?.large,turning=!!target?.junction?.decision,roundabout=!!target?.junction?.roundabout,arriving=!!target?.arrivalApproach,stopped=!!target?.stopped,instant=!!target?.instant&&ts-(cameraTargetContext?.updatedAt||ts)<420,top=navCameraMode==='top'||intent==='toggle-2d',preview=!!target?.previewing||intent==='preview',phase=target?.motion?.phase||'steady',rerouting=navCameraState===NAV_CAMERA_STATES.REROUTING;
  if(preview)return moto?{label:'preview-moto',posTau:118,viewTau:142,bearingTau:150,centerEps:.08,zoomEps:.0038,bearingEps:.17}:{label:'preview',posTau:150,viewTau:172,bearingTau:185,centerEps:.09,zoomEps:.004,bearingEps:.22};
  if(rerouting)return moto?{label:'rerouting-moto',posTau:118,viewTau:148,bearingTau:220,centerEps:.075,zoomEps:.0038,bearingEps:.16}:{label:'rerouting',posTau:158,viewTau:190,bearingTau:320,centerEps:.085,zoomEps:.004,bearingEps:.19};
  if(top)return moto?{label:'top-moto',posTau:instant?78:112,viewTau:instant?92:134,bearingTau:135,centerEps:.07,zoomEps:.0035,bearingEps:.16}:{label:'top',posTau:instant?92:132,viewTau:instant?112:158,bearingTau:160,centerEps:.08,zoomEps:.004,bearingEps:.2};
  if(intent==='launch')return moto?{label:'launch-moto',posTau:72,viewTau:88,bearingTau:98,centerEps:.06,zoomEps:.0028,bearingEps:.12}:{label:'launch',posTau:86,viewTau:108,bearingTau:132,centerEps:.07,zoomEps:.003,bearingEps:.16};
  if(intent==='recenter'||intent==='follow')return moto?{label:'recenter-moto',posTau:76,viewTau:94,bearingTau:104,centerEps:.06,zoomEps:.0028,bearingEps:.12}:{label:'recenter',posTau:94,viewTau:118,bearingTau:142,centerEps:.07,zoomEps:.003,bearingEps:.16};
  if(intent==='toggle-3d')return moto?{label:'toggle-3d-moto',posTau:82,viewTau:102,bearingTau:112,centerEps:.07,zoomEps:.003,bearingEps:.14}:{label:'toggle-3d',posTau:102,viewTau:126,bearingTau:150,centerEps:.08,zoomEps:.0035,bearingEps:.18};
  if(arriving)return moto?{label:'arrival-moto',posTau:118,viewTau:150,bearingTau:180,centerEps:.068,zoomEps:.0034,bearingEps:.16}:{label:'arrival',posTau:138,viewTau:172,bearingTau:215,centerEps:.075,zoomEps:.0038,bearingEps:.19};
  if(roundabout)return moto?{label:'roundabout-moto',posTau:86,viewTau:108,bearingTau:118,centerEps:.07,zoomEps:.0034,bearingEps:.14}:{label:'roundabout',posTau:118,viewTau:148,bearingTau:170,centerEps:.08,zoomEps:.004,bearingEps:.20};
  if(largeTurn)return moto?{label:'junction-moto',posTau:90,viewTau:114,bearingTau:124,centerEps:.07,zoomEps:.0035,bearingEps:.14}:{label:'junction',posTau:122,viewTau:152,bearingTau:180,centerEps:.08,zoomEps:.004,bearingEps:.20};
  if(turning)return moto?{label:'turn-moto',posTau:98,viewTau:124,bearingTau:138,centerEps:.072,zoomEps:.0036,bearingEps:.15}:{label:'turn',posTau:132,viewTau:162,bearingTau:195,centerEps:.082,zoomEps:.0042,bearingEps:.21};
  if(stopped||target?.nearlyStopped)return moto?{label:'stopped-moto',posTau:230,viewTau:300,bearingTau:820,centerEps:.045,zoomEps:.0026,bearingEps:.10}:{label:'stopped',posTau:250,viewTau:320,bearingTau:680,centerEps:.055,zoomEps:.0028,bearingEps:.12};
  if(phase==='braking')return moto?{label:'braking-moto',posTau:92,viewTau:116,bearingTau:185,centerEps:.065,zoomEps:.003,bearingEps:.14}:{label:'braking',posTau:132,viewTau:158,bearingTau:270,centerEps:.075,zoomEps:.0038,bearingEps:.18};
  if(phase==='accelerating')return moto?{label:'accelerating-moto',posTau:104,viewTau:132,bearingTau:158,centerEps:.072,zoomEps:.0035,bearingEps:.15}:{label:'accelerating',posTau:145,viewTau:178,bearingTau:250,centerEps:.082,zoomEps:.004,bearingEps:.19};
  if(phase==='crawl')return moto?{label:'crawl-moto',posTau:155,viewTau:205,bearingTau:420,centerEps:.055,zoomEps:.003,bearingEps:.11}:{label:'crawl',posTau:190,viewTau:235,bearingTau:470,centerEps:.065,zoomEps:.0035,bearingEps:.14};
  if(speed>14)return moto?{label:'fast-moto',posTau:122,viewTau:154,bearingTau:192,centerEps:.078,zoomEps:.0038,bearingEps:.16}:{label:'fast',posTau:155,viewTau:192,bearingTau:285,centerEps:.09,zoomEps:.0042,bearingEps:.20};
  if(speed>6)return moto?{label:'cruise-moto',posTau:132,viewTau:168,bearingTau:212,centerEps:.074,zoomEps:.0038,bearingEps:.15}:{label:'cruise',posTau:170,viewTau:205,bearingTau:315,centerEps:.082,zoomEps:.0042,bearingEps:.19};
  if(instant)return moto?{label:'instant-moto',posTau:72,viewTau:90,bearingTau:100,centerEps:.06,zoomEps:.0028,bearingEps:.12}:{label:'instant',posTau:88,viewTau:108,bearingTau:128,centerEps:.07,zoomEps:.003,bearingEps:.16};
  return moto?{label:'steady-moto',posTau:145,viewTau:182,bearingTau:235,centerEps:.068,zoomEps:.0038,bearingEps:.15}:{label:'steady',posTau:182,viewTau:220,bearingTau:345,centerEps:.078,zoomEps:.0042,bearingEps:.20};
}
...
const ALERT_ART_ASSETS=Object.freeze({});
...
function alertPriorityScore(a){const c=String(a?.category||'').toLowerCase(),base={robbery:100,harassment:96,road_block:90,accident:88,speed_camera:80,blitz:78,road_hazard:74,object_on_road:72,stopped_vehicle:68,traffic:60,broken_signal:58,poor_lighting:54,construction:48,flood:46,crowd:42}[c]||35;return base+(Math.min(5,+a?.severity||0)*3)+Math.min(12,+a?.confirmations||0)+Math.min(8,(+a?.confidence||0)*8)}
function syncAlertDomMarkers(items=lastAlertRecords){
  ...
  const visible=(items||[]).filter(a=>Number.isFinite(+a.lat)&&Number.isFinite(+a.lon)).slice().sort((a,b)=>alertPriorityScore(b)-alertPriorityScore(a)).slice(0,ALERT_DOM_MARKER_MAX),wanted=new Set(visible.map(a=>String(a.id)));
...
function openSafetyDrawer(force=true){const box=$('safetyDrawer'),btn=$('safetyToolsBtn');if(!box)return;const show=force===null?!box.classList.contains('show'):!!force;if(show){openPrefsDrawer(false);openAccountDrawer(false);hideResults()}box.classList.toggle('show',show);box.setAttribute('aria-hidden',String(!show));btn?.classList.toggle('safety-open',show);if(show&&window.lucide)lucide.createIcons()}
...
function syncSearchResultsPlacement(open=results?.classList.contains('show')){
  ...
  if(!open){
    ...
    ['left','right','top','bottom','width','max-height'].forEach(k=>results.style.removeProperty(k));
    return;
  }
...
function searchResultBadge(x){if(x.match_kind==='primary')return'MELHOR';if(x.match_kind==='related')return'RELACIONADO';if(x.address_number_match==='matched'&&x.postcode_match==='matched')return'EXATO';if(x.match_confidence==='exact')return'EXATO';if(x.accuracy==='rooftop')return'PRECISO';if(x.accuracy==='interpolated')return'APROX.';if(x.source==='cep-authoritative')return'CEP';if(x.source==='cep-fallback')return'APROX.';if(x.type==='address')return'ENDEREÇO';if(x.type==='poi')return'LOCAL';return''}
...
function resultHtml(list,title='Melhores resultados'){
  ...
  return `<div class="search-label"><span>${esc(title)}</span><em>${visible.length} ${visible.length===1?'resultado':'resultados'}</em></div>`+visible.map((x,i)=>{
    ...
    const matchNote=x.match_reason?`<span class="result-match-note ${x.match_kind==='primary'?'primary':''}">${x.match_kind==='primary'?'<i data-lucide="sparkles" width="12"></i>':''}${esc(x.match_reason)}</span>`:'';
    const matchClass=x.match_kind==='primary'?'match-primary':x.match_kind==='related'?'match-related':'';
    return `<button type="button" role="option" class="search-item rich ${hasPhoto?'has-photo':'no-photo'} ${matchClass}" data-i="${i}">${visual}<span class="result-copy"><div class="result-topline"><b>${esc(p.primary)}</b></div><span class="result-address">${esc(p.secondary)}</span>${matchNote}<small>${esc(searchResultMeta(x))}</small>${rating?`<span class="result-stars">${rating}</span>`:''}</span>${badge?`<span class="result-meta ${badge==='EXATO'||badge==='PRECISO'||badge==='MELHOR'?'precise':''}">${esc(badge)}</span>`:''}</button>`;
  }).join('')
}
...
function animatedRouteFlowGradient(phase=0,{base=VANO_ROUTE,accent=VANO_ROUTE_LIGHT,pulse='#FFF7F2',segments=7,pulseWidth=.085}={}){
...
function syncNavigationThemePaint(){
  ...
  try{
    if(map.getLayer('nav-route-casing'))map.setPaintProperty('nav-route-casing','line-color',black?'rgba(255,255,255,.86)':'rgba(255,255,255,.98)');
    if(map.getLayer('nav-route-casing'))map.setPaintProperty('nav-route-casing','line-opacity',black?.76:.88);
...
function updateAnimatedRoutePaint(force=false){
  ...
  try{
    if(map.getLayer('route-selected-flow'))map.setPaintProperty('route-selected-flow','line-gradient',animatedRouteFlowGradient(planPhase,{base:VANO_ROUTE,accent:VANO_ROUTE_LIGHT,pulse:'#FFD7AD',segments:8,pulseWidth:.075}));
    if(map.getLayer('nav-route-flow'))map.setPaintProperty('nav-route-flow','line-gradient',animatedRouteFlowGradient(navPhase,{base:VANO_ROUTE,accent:VANO_ROUTE_LIGHT,pulse:'#FFE2B8',segments:9,pulseWidth:.058}));
...
async function installRuntimeLayers(){
  ...
  ensureLayer({id:'nav-route-flow',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':animatedRouteFlowGradient(0,{base:VANO_ROUTE,accent:VANO_ROUTE_LIGHT,pulse:'#FFE2B8',segments:8,pulseWidth:.058}),'line-width':['interpolate',['linear'],['zoom'],10,1.5,12,1.9,14,2.3,16,2.8,18,3.3,20,3.8],'line-opacity':.42}},routeBefore);
  ...
  ensureLayer({id:'nav-turn-core',type:'line',source:'nav-turn',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#FFFBF2','line-width':['interpolate',['linear'],['zoom'],12,2.2,16,3.2,20,4.2],'line-opacity':.96}},routeBefore);
  ensureGeoSource('route-progress');
  ensureLayer({id:'route-progress',type:'line',source:'route-progress',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#f6f7f9','line-width':['interpolate',['linear'],['zoom'],10,4.2,12,5.2,14,6.2,16,7.5,18,8.8,20,10.2],'line-opacity':.72}},routeBefore);
...
function setNavigationFollow(enabled,recenter=true){followMode=!!enabled;setNavCameraState(followMode?(recenter?NAV_CAMERA_STATES.RECENTERING:NAV_CAMERA_STATES.FOLLOWING):NAV_CAMERA_STATES.FREE_LOOK);if(!followMode)stopCameraMotion();else if(recenter)markCameraIntent('follow',960);syncFollowButton();document.body.classList.toggle('nav-map-free',!followMode&&!!activeNav?.classList.contains('show'));syncNavigationTripHud();if(followMode&&recenter&&lastNavPosition)scheduleCamera(lastNavPosition,true,nearestProgress(lastNavPosition))}
...
function animateCalibrationButton(duration=900){
  const ids=['navDrawerRecenter','recenterBtn','navRecenter'];
  ids.forEach(id=>{const b=$(id);if(!b)return;b.classList.add('calibrating');b.setAttribute('aria-busy','true')});
  if(cameraCalibrationTimer)clearTimeout(cameraCalibrationTimer);
  cameraCalibrationTimer=setTimeout(()=>{ids.forEach(id=>{const b=$(id);if(!b)return;b.classList.remove('calibrating');b.removeAttribute('aria-busy')});cameraCalibrationTimer=null},Math.max(420,+duration||900));
}
async function calibrateNavigation(){
  const navOn=!!activeNav?.classList.contains('show');
  // Recalibration must always leave any free-look/search interaction state first.
  try{setSearchInteraction(false)}catch{searchInteractionActive=false}
  followMode=true;mapFollowMode=true;navCameraMode='perspective';setNavCameraState(NAV_CAMERA_STATES.RECENTERING);document.body.classList.remove('nav-map-free');syncNavCameraButton();syncFollowButton();animateCalibrationButton(980);

  // Android/native navigation owns its own camera. The old implementation only
  // moved the web Mapbox camera, so this button looked completely broken there.
  if(window.__vanoNativeMapActive&&navOn){
    const ok=vanoNativeMapRecenter();haptic(8);setTimeout(()=>haptic(4),120);
    showToast(ok?'Câmera recalibrada.':'Recalibrando câmera…');
    return ok;
  }

  let pos=lastNavPosition||lastPassivePosition||userLocation||readLastGps();
  if(!pos||!Number.isFinite(+pos.lat)||!Number.isFinite(+pos.lon)){
    await locateUser(false,{centerMap:false,keepFollow:true});
    pos=lastNavPosition||lastPassivePosition||userLocation||readLastGps();
  }
  if(!pos||!Number.isFinite(+pos.lat)||!Number.isFinite(+pos.lon)){
    setNavCameraState(NAV_CAMERA_STATES.FOLLOWING);showToast('Ainda não temos um sinal de GPS para recalibrar.');return false;
  }

  // Outside an active route the same control simply recenters on the best fix.
  if(!navOn||!selectedRoute?.geometry?.coordinates?.length){
    try{map?.stop?.();map?.easeTo?.({center:[+pos.lon,+pos.lat],zoom:16.3,pitch:0,bearing:0,padding:{top:0,bottom:0,left:0,right:0},retainPadding:false,duration:430,essential:true,easing:t=>1-Math.pow(1-t,3)})}catch{}
    setNavCameraState(NAV_CAMERA_STATES.FOLLOWING);haptic(7);showToast('Mapa centralizado no seu GPS.');return true;
  }
  ...
  try{
    ...
    map?.easeTo?.({center:target.center,zoom:target.zoom,pitch:target.pitch,bearing:target.bearing,padding:target.padding,retainPadding:false,duration:480,essential:true,easing:t=>t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2});
...
function navigationTripStatus(){if(navCameraState===NAV_CAMERA_STATES.FREE_LOOK)return'CÂMERA LIVRE · TOQUE NO ALVO PARA VOLTAR';if(navCameraState===NAV_CAMERA_STATES.RECENTERING)return'RECENTRALIZANDO CÂMERA';if(navCameraState===NAV_CAMERA_STATES.REROUTING)return'RECALCULANDO ROTA';if(navCameraState===NAV_CAMERA_STATES.ARRIVAL)return'CHEGANDO AO DESTINO';if(navCameraState===NAV_CAMERA_STATES.MANEUVER_FOCUS)return'MANOBRA À FRENTE';return'EM ROTA · CÂMERA SEGUINDO'}
function syncNavigationTripHud(progressInfo=null){const name=$('navDestinationName'),status=$('navTripStatus'),bar=$('navTripProgress');if(name){const raw=String(destination?.name||destination?.label||'Destino').trim();name.textContent=(raw.split(',')[0]||raw||'Destino').slice(0,72)}if(status)status.textContent=navigationTripStatus();if(bar){let progress=Number.isFinite(+progressInfo?.progress)?+progressInfo.progress:null;if(progress===null&&lastNavPosition&&selectedRoute){try{progress=nearestProgress(lastNavPosition).progress}catch{progress=null}}progress=Math.max(0,Math.min(1,Number.isFinite(progress)?progress:0));const pct=Math.round(progress*100),track=bar.parentElement;bar.style.width=`${(progress*100).toFixed(1)}%`;track?.classList.toggle('complete',progress>.985);track?.setAttribute('role','progressbar');track?.setAttribute('aria-label','Progresso da viagem');track?.setAttribute('aria-valuemin','0');track?.setAttribute('aria-valuemax','100');track?.setAttribute('aria-valuenow',String(pct));track?.setAttribute('aria-valuetext',`${pct}% da viagem concluída`)}}
function updateNavSummary(etaSeconds,remainingMeters,step,progressInfo=null){const mins=$('remainingMinutes'),km=$('remainingKm'),next=$('nextDistance'),confidence=$('navEtaConfidence');if(mins)mins.textContent=navMinutesLabel(etaSeconds);if(km)km.textContent=navKmLabel(remainingMeters);if(next){const d=Math.max(0,+step?.remainingInStep||0);next.textContent=d<18?'Agora':fmtDistance(d);const prefix=$('nextDistancePrefix');if(prefix)prefix.textContent=d<18?'':'Em '}if(confidence)confidence.textContent=etaBandLabel(etaSeconds,selectedRoute);updateFollowingManeuver(step);syncNavigationTripHud(progressInfo);syncPremiumRouteCards()}
async function returnToDestinationSearch(){window.__vanoRouteMore?.close?.();await finishTrip(false);destinationConfirmed=false;if(!LOGGED_IN)guestTrialId='';destination=null;routes=[];selectedRoute=null;manualRouteSelection=false;if(destinationMarker){destinationMarker.remove();destinationMarker=null}try{map.getSource('routes')?.setData(emptyFC());map.getSource('route-alternatives')?.setData(emptyFC());map.getSource('nav-route')?.setData(emptyFC());map.getSource('traffic-live-segments')?.setData(emptyFC())}catch{}routeState.style.display='none';welcomeState.style.display='block';destinationInput.value='';planSheet.classList.remove('hidden','sheet-dragging');planSheet.classList.add('sheet-collapsed');planSheet.style.transform='';hideResults();requestAnimationFrame(()=>{syncFloatingLocate();destinationInput.focus({preventScroll:true});hideResults()})}
...
function resolveVehicleSpeedKmh(raw,p){
  ...
  let targetKmh=Math.max(0,targetMps*3.6);if(targetKmh<5.5)targetKmh=0;if(targetKmh>0)lastSpeedMotionAt=now;if(targetKmh===0&&now-lastSpeedMotionAt<800)return stableNavSpeedKmh;const alpha=targetKmh>=stableNavSpeedKmh?.32:.24;stableNavSpeedKmh=stableNavSpeedKmh+(targetKmh-stableNavSpeedKmh)*alpha;if(stableNavSpeedKmh<2.2)stableNavSpeedKmh=0;return stableNavSpeedKmh
}
...
function resumeModeLabel(mode){return mode==='fastest'?'Mais rápida':mode==='smart'?'Spark':'Mais segura'}
...
async function continueSavedTrip(){const d=pendingResumeTrip||readSavedActiveTrip();if(!d)return;const btn=$('continueResumeTrip');if(btn){btn.disabled=true;btn.textContent='Retomando…'}try{
  profile=['walking','driving','motorcycle','cycling'].includes(d.profile)?d.profile:'driving';syncRouteProfileUi();routeMode=['fastest','safest','smart'].includes(d.routeMode)?d.routeMode:'safest';origin=d.origin||origin;destination=d.destination;selectedRoute=d.route;routes=[selectedRoute];destinationConfirmed=true;manualRouteSelection=false;
...
window.__VANO_MAP_BOOT_STAGE='map-created';window.__VANO_MAP_INSTANCE_READY=true;setTimeout(()=>{prefetchMapStyle(mapStyleForMood('day'));prefetchMapStyle(mapStyleForMood('night'));prefetchMapStyle(mapStyleForMood('black'))},250);window.__VANO_MAP_BRIDGE={version:314,getMap:()=>map,getUserLocation:()=>{const p=userLocation;return p&&Number.isFinite(+p.lat)&&Number.isFinite(+p.lon)?{lat:+p.lat,lon:+p.lon}:null},getOsintSnapshot:()=>({navigation:!!activeNav?.classList?.contains('show'),location:(lastNavPosition||userLocation)?osintLocationPayload(lastNavPosition||userLocation,lastNavPosition||userLocation):null,route:(activeNav?.classList?.contains('show')&&selectedRoute)?osintRoutePayload(lastNavPosition||userLocation,lastNavPosition?nearestProgress(lastNavPosition):null,null):null}),isPlanning:()=>!!destination||!!destinationConfirmed||!!selectedRoute||!!activeNav?.classList?.contains('show'),getPerformanceTier:()=>performanceTier,getNavigationSnapshot:()=>({active:!!activeNav?.classList?.contains('show'),routeMode,profile,selectedRoute:selectedRoute?{id:selectedRoute.id,duration:selectedRoute.duration,distance:selectedRoute.distance,safety_score:routeSafetyScore(selectedRoute),traffic_score:+selectedRoute.traffic_score||0}:null,progress:lastNavPosition&&selectedRoute?nearestProgress(lastNavPosition):null,online:navigator.onLine!==false}),setSmartBalance:v=>{const input=$('smartBalance');if(input)input.value=String(Math.max(0,Math.min(100,+v||0)));applySmartBalance(v,{recalculate:true});return true},openDestination:async p=>{if(!p||!Number.isFinite(+p.lat)||!Number.isFinite(+p.lon))return false;if(activeNav?.classList?.contains('show'))await returnToDestinationSearch();planSheet?.classList.remove('hidden');planSheet?.classList.remove('sheet-collapsed');if(welcomeState)welcomeState.style.display='block';if(routeState)routeState.style.display='none';setPoint('destination',{...p,lat:+p.lat,lon:+p.lon,label:String(p.label||'Destino'),name:String(p.name||String(p.label||'Destino').split(',')[0]),type:p.type||'routine'},String(p.label||'Destino'),true);return true}};window.dispatchEvent(new CustomEvent('vano:map-ready'));
...
function chooseByMode(){manualRouteSelection=false;const badge=['fastest','safest','smart'].includes(routeMode)?routeMode:'safest';selectedRoute=routes.find(r=>r.badges?.includes(badge))||routes.find(r=>r.badges?.includes('smart'))||routes.find(r=>r.badges?.includes('safest'))||routes[0]||null}
function routeName(){return routeMode==='fastest'?'Rota mais rápida':routeMode==='smart'?'Rota Spark':'Rota mais segura'}
...
function routeCacheKey(start,endLat,endLon,modeOverride=routeMode){const mode=['fastest','safest','smart'].includes(modeOverride)?modeOverride:'safest',moving=isMotorizedProfile()&&Number.isFinite(+start?.heading)&&Number.isFinite(+start?.speed)&&+start.speed>=1.2,dir=moving?`${Math.round((((+start.heading%360)+360)%360)/10)*10}:${Math.round(+start.speed)}`:'na';return [profile,mode,(+start.lat).toFixed(5),(+start.lon).toFixed(5),(+endLat).toFixed(5),(+endLon).toFixed(5),dir,start?.is_reroute?1:0,mapPrefs.adaptiveRoutes?1:0,mapPrefs.aggressiveShortcuts?1:0,NAV_PREFS.avoid_ferries?1:0,NAV_PREFS.avoid_tolls?1:0,NAV_PREFS.avoid_unpaved?1:0].join(':')}
function routeRequestInfo(start,dest=destination,modeOverride=routeMode){
  if(!start||!dest)return null;
  const mode=['fastest','safest','smart'].includes(modeOverride)?modeOverride:'safest',hasEntrance=dest.entrance_lat!=null&&dest.entrance_lon!=null&&Number.isFinite(+dest.entrance_lat)&&Number.isFinite(+dest.entrance_lon),endLat=profile==='walking'&&hasEntrance?+dest.entrance_lat:+dest.lat,endLon=profile==='walking'&&hasEntrance?+dest.entrance_lon:+dest.lon,key=routeCacheKey(start,endLat,endLon,mode);
...
async function prefetchDestinationRoutes(candidate){
  ...
  const primaryMode=['fastest','safest','smart'].includes(routeMode)?routeMode:'safest',secondaryModes=['safest','fastest','smart'].filter(x=>x!==primaryMode);
  const primary=prefetchRouteMode(candidate,primaryMode,start);
  secondaryModes.forEach(mode=>prefetchRouteMode(candidate,mode,start).catch(()=>{}));
  return primary;
}
...
function maybeSpeakStep(step){
  ...
  const speed=Math.max(0,+lastNavPosition?.speed||0),far=isMotorizedProfile()&&speed>13?620:420,mid=isMotorizedProfile()&&speed>8?220:160,threshold=d<=45?45:d<=mid?mid:d<=far?far:null;if(!threshold)return;
...
function speak(text,key=text){
  if(!soundEnabled||!text||key===lastSpokenInstruction)return;const kind=navAudioKind(text,key);if(!canPlayNavAudio(kind,String(key)))return;lastSpokenInstruction=key;
...
function openPrefsDrawer(force){
  const d=$('prefsDrawer');if(!d)return;
  const open=typeof force==='boolean'?force:!d.classList.contains('show');
  if(open){openAccountDrawer(false);openSafetyDrawer(false);hideResults();destinationInput?.blur();syncPrefsUI();populateVoiceSelectors()}
  d.classList.toggle('show',open);d.setAttribute('aria-hidden',String(!open));
  $('prefsBtn')?.classList.toggle('active',open);$('prefsBtn')?.setAttribute('aria-expanded',String(open));
}
...
function clearLiveRoad(){liveContext=null;lastLiveContextAt=0;lastLiveContextPos=null}
...
function setPreviewUi(active){const btn=$('previewRouteBtn'),cancel=$('previewCancelBtn');btn?.classList.toggle('previewing',!!active);if(btn)btn.innerHTML=active?'<i data-lucide="move-3d" width="14"></i><span>Prévia ativa</span>':'<i data-lucide="move-3d" width="14"></i><span>Prévia 3D</span>';cancel?.classList.toggle('show',!!active);document.body.classList.toggle('route-previewing',!!active);if(window.lucide)lucide.createIcons();if(active)requestAnimationFrame(syncPreviewCancelPosition)}
...
async function activateNavigationAt(p,{simulated=false}={}){
  ...
  if(!simulated){nonFatal('wake-lock',()=>requestWake())}else $('simBadge')?.classList.add('show');
...
function ensureAdminSimulationControl(){
  if(!IS_ADMIN)return null;let btn=$('adminSimBtn');if(btn)return btn;const host=$('adminRouteTestSlot')||routeState?.querySelector('.vr-hero')?.parentElement||routeState;if(!host)return null;const slot=$('adminRouteTestSlot')||document.createElement('div');if(!slot.id){slot.id='adminRouteTestSlot';slot.className='admin-route-test-slot';const hero=routeState?.querySelector('.vr-hero');hero?.insertAdjacentElement('afterend',slot)}slot.innerHTML='<button class="admin-route-test-btn" id="adminSimBtn" type="button" aria-label="Testar rota como administrador"><span class="admin-route-test-icon"><i data-lucide="flask-conical" width="17"></i></span><span class="admin-route-test-copy"><b>Testar rota</b><small>Admin · simula movimento, câmera, manobras e chegada</small></span><span class="admin-route-test-badge">TESTE</span></button>';btn=$('adminSimBtn');if(window.lucide)lucide.createIcons();return btn
}
function bindRouteMoreDetails(){
  const details=$('routeMoreDetails'),toggle=$('routeMoreToggle'),content=$('routeMoreContent');if(!details||!toggle||!content)return;
  let busy=false;
  const setOpen=(open,{scroll=true,focus=false}={})=>{
    open=!!open;details.dataset.open=open?'true':'false';details.classList.toggle('is-open',open);toggle.setAttribute('aria-expanded',String(open));content.hidden=!open;content.setAttribute('aria-hidden',String(!open));planSheet?.classList.toggle('route-details-open',open);
    requestAnimationFrame(()=>{
      try{syncFloatingLocate()}catch{}try{map?.resize?.()}catch{}
      if(open&&scroll&&planSheet){
        const sheetRect=planSheet.getBoundingClientRect(),detailsRect=details.getBoundingClientRect(),safeTop=sheetRect.top+36,safeBottom=sheetRect.bottom-18;
        let target=planSheet.scrollTop;if(detailsRect.top<safeTop)target-=safeTop-detailsRect.top;else if(detailsRect.bottom>safeBottom)target+=Math.min(detailsRect.bottom-safeBottom,Math.max(80,planSheet.clientHeight*.42));
        if(Math.abs(target-planSheet.scrollTop)>4){try{planSheet.scrollTo({top:Math.max(0,target),behavior:REDUCED_MOTION?'auto':'smooth'})}catch{planSheet.scrollTop=Math.max(0,target)}}
      }
      if(focus)toggle.focus({preventScroll:true});
    });
  };
  const toggleOpen=e=>{e?.preventDefault?.();e?.stopPropagation?.();if(busy)return;busy=true;setOpen(details.dataset.open!=='true');setTimeout(()=>busy=false,REDUCED_MOTION?0:180)};
  toggle.addEventListener('click',toggleOpen);toggle.addEventListener('pointerdown',e=>e.stopPropagation());
  details.addEventListener('click',e=>e.stopPropagation());
  details.addEventListener('keydown',e=>{if(e.key==='Escape'&&details.dataset.open==='true'){e.preventDefault();setOpen(false,{scroll:false,focus:true})}});
  window.__vanoRouteMore={open:()=>setOpen(true),close:()=>setOpen(false,{scroll:false}),toggle:()=>setOpen(details.dataset.open!=='true')};
  setOpen(false,{scroll:false});
}
...
function updateNavigation(g,simulated=false){
  ...
  document.body.classList.toggle('nav-approaching-arrival',m.remaining<420&&m.progress>.80);if(now-lastNavUiAt>240){lastNavUiAt=now;$('maneuverGlyph').textContent=maneuverGlyph(step);$('nextInstruction').textContent=m.remaining<22&&m.progress>.92?'Você chegou':maneuverLabel(step);$('nextStreet').textContent=m.remaining<22?'Destino':stepStreet(step);$('arrivalTime').textContent=fmtClock(new Date(Date.now()+eta*1000));$('currentSpeed').textContent=Number.isFinite(+currentKmh)?`${Math.round(currentKmh)} km/h`:'— km/h';updateNavSummary(eta,m.remaining,step,m);renderLaneGuidance(step);renderRiskAhead(m);renderRoadAwareness(p,m);updateNavContextAlert(m,step);}
  syncManeuverCardDensity(step);if(Math.abs(navLastAlong-navLastPaintAlong)>1.8||now-lastNavRoutePaintAt>420){lastNavRoutePaintAt=now;navLastPaintAlong=navLastAlong;setNavRouteData(navLastAlong)}if(now-lastProgressPaintAt>280){lastProgressPaintAt=now;updateTurnHighlight(m.distanceAlong,step)}
  refreshNavigationAlternatives(false);updateTrafficRadar(false,m.distanceAlong);
  if(!simulated){const rawOff=Number.isFinite(rawMatch?.offRoute)?rawMatch.offRoute:m.offRoute;const shouldReroute=updateOffRouteTracker(raw,rawOff);if(shouldReroute)rerouteFrom(filtered);checkLiveTraffic(false);checkEventDisruption(false,{pretrip:false});pushLivePosition(p,m,false);maybeSyncPresence(p)}
...
function openQuickAlert(force=null){const sheet=$('quickAlertSheet');if(!sheet)return;const show=force===null?!sheet.classList.contains('show'):!!force;sheet.classList.toggle('show',show);sheet.setAttribute('aria-hidden',String(!show));if(show){openPrefsDrawer(false);openSafetyDrawer(false);openAccountDrawer(false);hideResults();const status=$('quickAlertStatus');if(status)status.textContent='';setNavControlDrawer(false);haptic(5);if(window.lucide)lucide.createIcons()}}
...
async function hydratePlannerSmartDestinations(){const host=$('plannerSmartDestinations'),list=$('plannerSmartDestinationList');if(!host||!list)return;let remote=[];if(LOGGED_IN){try{remote=await loadSearchSuggestions(false)}catch{remote=[]}}const local=localSmartDestinations(),seen=new Set(),usable=[];for(const x of [...(remote||[]).filter(x=>['smart','recent','favorite'].includes(x.kind)),...local]){if(!Number.isFinite(+x.lat)||!Number.isFinite(+x.lon))continue;const key=`${(+x.lat).toFixed(4)}:${(+x.lon).toFixed(4)}`;if(seen.has(key))continue;seen.add(key);usable.push(x);if(usable.length>=6)break}if(!usable.length){host.hidden=true;return}host.hidden=false;list.innerHTML=usable.map((x,i)=>`<button type="button" data-smart-dest="${i}"><span><i data-lucide="${x.kind==='favorite'?'star':x.kind==='smart'?'sparkles':'history'}" width="14"></i></span><b>${esc(x.name||String(x.label||'Destino').split(',')[0])}</b><small>${x.kind==='smart'?'Bom para este horário':x.kind==='favorite'?'Favorito':x.kind==='local'?`${+x.uses||1} uso(s) neste aparelho`:`${+x.uses||1} visita(s)`}</small></button>`).join('');list.querySelectorAll('[data-smart-dest]').forEach(btn=>btn.addEventListener('click',()=>{const x=usable[+btn.dataset.smartDest];if(x)setPoint('destination',{...x,lat:+x.lat,lon:+x.lon,label:x.label,name:x.name||String(x.label||'Destino').split(',')[0]},x.label,true)}));if(window.lucide)lucide.createIcons()}
setTimeout(hydratePlannerSmartDestinations,700);
const smartBalance=$('smartBalance');if(smartBalance){syncSmartBalanceUI();let smartBalanceTimer=null;smartBalance.addEventListener('input',()=>{clearTimeout(smartBalanceTimer);applySmartBalance(smartBalance.value,{recalculate:false});smartBalanceTimer=setTimeout(()=>applySmartBalance(smartBalance.value,{recalculate:true}),260)})}
...
[originInput,destinationInput].forEach(inp=>{
  ...
  inp.addEventListener('focus',()=>{activeSearchKind=kind;setSearchInteraction(true);planSheet?.classList.add('planner-search-active');if(kind==='destination'&&window.innerWidth<900&&welcomeState?.style.display!=='none'){planSheet?.classList.add('sheet-collapsed');const grab=$('planGrab');grab?.setAttribute('aria-expanded','false');grab?.setAttribute('aria-label','Expandir painel de destino');if(grab)grab.dataset.state='collapsed'}lockIOSInputViewport(inp);refreshResponsiveViewport();requestAnimationFrame(()=>{lockIOSInputViewport(inp);syncFloatingLocate();syncSearchResultsPlacement()});if(inp.value.trim().length>=SEARCH_FAST_MIN&&!searchComposing)queueSearch(inp,kind);else{hideResults();if(kind==='destination')showSavedSearchSuggestions(kind)}});
...
bindClick('prefsBtn',()=>openPrefsDrawer(null));bindClick('prefsCloseBtn',()=>openPrefsDrawer(false));bindClick('prefsDoneBtn',()=>openPrefsDrawer(false));bindPref('prefAdaptive','adaptiveRoutes',true);bindPref('prefAggressive','aggressiveShortcuts',true);bindPref('prefAlternatives','showAlternatives',false);bindPref('prefLiveSignals','liveSignals',false);bindPref('prefAutoFaster','autoFaster',false);syncPrefsUI();
...
ensureAdminSimulationControl();bindRouteMoreDetails();bindClick('continueResumeTrip',continueSavedTrip);bindClick('discardResumeTrip',discardSavedTrip);bindClick('arrivalFinishBtn',()=>finishTrip(false));bindClick('arrivalParkingBtn',openArrivalParking);bindClick('confirmDestinationBtn',confirmDestination);bindClick('editDestinationBtn',editDestination);bindClick('previewRouteBtn',previewRoute);bindClick('adminSimBtn',startAdminSimulation,true);bindClick('previewCancelBtn',()=>stopRoutePreview(true));bindClick('navAltAccept',acceptNavigationAlternative);bindClick('navAltDismiss',dismissNavigationAlternative);bindClick('navSearchBtn',returnToDestinationSearch);bindClick('navExitBtn',()=>finishTrip(false));syncNavCameraButton();syncFollowButton();syncImmersiveButton();syncSoundButton();populateVoiceSelectors();bindClick('quickAlertBtn',()=>openQuickAlert(null));bindClick('quickAlertCloseBtn',()=>openQuickAlert(false));bindClick('quickAlertBackdrop',()=>openQuickAlert(false));document.querySelectorAll('[data-quick-alert]').forEach(btn=>btn.addEventListener('click',()=>submitQuickAlert(btn.dataset.quickAlert,btn)));bindClick('safetyToolsBtn',()=>openSafetyDrawer(null),true);bindClick('safetyCloseBtn',()=>openSafetyDrawer(false));bindClick('safetyLiveBtn',toggleLiveShare);bindClick('supportPointsBtn',loadSupportPoints);bindClick('emergencyShareBtn',()=>shareSafetyMessage('sos'));bindClick('checkinBtn',()=>shareSafetyMessage('checkin'));bindClick('liveShareBtn',toggleLiveShare);bindClick('acceptTrafficRoute',applyTrafficSuggestion);bindClick('dismissTrafficRoute',()=>{trafficDismissUntil=Date.now()+120000;hideTrafficSuggestion();showToast('Mantendo a rota atual.')});bindClick('navRecenter',()=>calibrateNavigation());bindClick('soundBtn',()=>{if(navVoiceHoldOpened){navVoiceHoldOpened=false;return}soundEnabled=!soundEnabled;try{if(!soundEnabled){window.speechSynthesis?.cancel();stopVanoVoice()}}catch{}toggleVoicePopover(false);syncSoundButton();showToast(soundEnabled?'Orientações por voz ativadas.':'Orientações por voz desativadas.')});bindClick('shareRouteBtn',shareRoute);bindClick('showAlertsBtn',()=>{if(!selectedRoute)return;const n=selectedRoute.nearby_alerts?.length||0;showToast(n?`${n} alerta(s) comunitário(s) próximo(s) desta rota.`:'Nenhum alerta recente próximo desta rota.')});
...
function syncFloatingLocate(){
  const button=$('recenterBtn'),app=$('wsApp');if(!button||!app)return;
  if(activeNav?.classList.contains('show')){button.style.removeProperty('bottom');syncNavigationHudGeometry();return}
  const rect=app.getBoundingClientRect();let bottom=24;
  for(const panel of [planSheet,$('destinationConfirm'),$('resumeTripCard')]){
    if(!panel||panel.classList.contains('hidden')||getComputedStyle(panel).visibility==='hidden'||getComputedStyle(panel).display==='none')continue;
    const box=panel.getBoundingClientRect();
    // On wide screens the sheet is on the left, away from this right-hand button.
    if(box.width&&box.right>rect.right-80)bottom=Math.max(bottom,rect.bottom-box.top+12);
  }
  const tools=document.querySelector('.map-tools')?.getBoundingClientRect();
  const maxBottom=Math.max(24,rect.height-(tools?.bottom-rect.top||170)-64);
  button.style.setProperty('bottom',`${Math.round(Math.min(bottom,maxBottom))}px`,'important');
  requestAnimationFrame(syncPreviewCancelPosition);
}
function syncNavigationHudGeometry(){
  const app=$('wsApp');if(!app||!activeNav?.classList.contains('show'))return;
  const appRect=app.getBoundingClientRect(),navRect=activeNav.getBoundingClientRect(),maneuver=$('navManeuverTop')?.getBoundingClientRect();
  if(!navRect.height)return;
  const set=(key,value)=>{const text=`${Math.round(value)}px`;if(app.style.getPropertyValue(key)!==text)app.style.setProperty(key,text)};
  set('--ui-hud-bottom',Math.max(12,appRect.bottom-navRect.top+12));
  if(maneuver)set('--ui-maneuver-bottom',Math.max(12,maneuver.bottom-appRect.top+10));
  const landscape=window.matchMedia('(max-height:520px) and (min-width:600px)').matches;
  let noticeTop=landscape?Math.max(12,parseFloat(getComputedStyle(app).paddingTop)||0):Math.max(12,(maneuver?.bottom||appRect.top+180)-appRect.top+10);
  for(const id of ['rerouteNotice','navConnectivityPill','navAltHint','navContextAlert']){
    const notice=$(id);if(!notice||notice.hidden||getComputedStyle(notice).display==='none'||getComputedStyle(notice).visibility==='hidden')continue;
    const value=`${Math.round(noticeTop)}px`;if(notice.style.getPropertyValue('--ui-notice-top')!==value)notice.style.setProperty('--ui-notice-top',value);
    noticeTop+=notice.getBoundingClientRect().height+8;
  }
  // Old versions wrote important inline offsets that defeated landscape CSS.
  for(const id of ['navStreetChip','navControlStack','vanoSpeedometerV220']){
    const el=$(id);for(const prop of ['left','right','top','bottom','transform'])el?.style.removeProperty(prop);
  }
}
...
function setupMapPanelAccessibility(){
  const host=$('wsApp'),backdrop=$('mapPanelBackdrop');
  const panels=[$('prefsDrawer'),$('safetyDrawer'),$('accountDrawer'),$('quickAlertSheet')].filter(Boolean);
  let active=null,returnFocus=null,inerted=[];
  const close=()=>{openPrefsDrawer(false);openSafetyDrawer(false);openAccountDrawer(false);openQuickAlert(false)};
  backdrop?.addEventListener('click',close);
  const focusable=panel=>[...panel.querySelectorAll('button:not(:disabled),a[href],input:not(:disabled),select:not(:disabled),[tabindex="0"]')].filter(el=>el.getClientRects().length&&!el.closest('[hidden]'));
  const restore=()=>{for(const [el,previous]of inerted)el.inert=previous;inerted=[]};
  const sync=()=>{
    const next=panels.find(panel=>panel.classList.contains('show'))||null;
    for(const panel of panels)panel.setAttribute('aria-hidden',String(panel!==next));
    if(backdrop)backdrop.hidden=!next||next.id==='accountDrawer';
    if(next===active)return;
    restore();
    if(next){
      if(!active)returnFocus=document.activeElement;
      active=next;
      for(const child of host.children){if(child===next||child===backdrop||child.id==='accountBackdrop'||child.id==='toast')continue;inerted.push([child,child.inert]);child.inert=true}
      requestAnimationFrame(()=>{if(active===next)(focusable(next)[0]||next).focus({preventScroll:true})});
    }else{
      active=null;const target=returnFocus;returnFocus=null;
      requestAnimationFrame(()=>{if(target?.isConnected&&!target.closest('[inert]'))target.focus?.({preventScroll:true});syncFloatingLocate()});
    }
  };
  const observer=new MutationObserver(sync);for(const panel of panels)observer.observe(panel,{attributes:true,attributeFilter:['class']});sync();
  document.addEventListener('keydown',event=>{
    if(!active)return;
    if(event.key==='Escape'){event.preventDefault();event.stopImmediatePropagation();close();return}
    if(event.key==='Tab'){
      const controls=focusable(active),first=controls[0],last=controls[controls.length-1];
  ...
  const choiceButtons=[...document.querySelectorAll('[data-route-mode],[data-profile]')];
  const syncChoices=()=>choiceButtons.forEach(button=>button.setAttribute('aria-pressed',String(button.classList.contains('active'))));
  const choices=new MutationObserver(syncChoices);choiceButtons.forEach(button=>choices.observe(button,{attributes:true,attributeFilter:['class']}));syncChoices();
  results?.addEventListener('keydown',event=>{
    const options=[...results.querySelectorAll('button.search-item,button.search-suggestion')],index=options.indexOf(document.activeElement);
    if(event.key==='Escape'){event.preventDefault();hideResults();destinationInput.focus({preventScroll:true});return}
    ...
    options[next]?.focus({preventScroll:true});options[next]?.scrollIntoView({block:'nearest',behavior:'auto'});
  });
}
setupMapPanelAccessibility();
const noticeObserver=new MutationObserver(()=>requestAnimationFrame(syncNavigationHudGeometry));
for(const id of ['rerouteNotice','navConnectivityPill','navAltHint','navContextAlert']){const node=$(id);if(node)noticeObserver.observe(node,{attributes:true,attributeFilter:['class','hidden'],childList:true,subtree:true})}
bindPlanningSheetDrag();bindAccountDrawerDrag();if(window.ResizeObserver&&planSheet){new ResizeObserver(()=>requestAnimationFrame(()=>{syncFloatingLocate();syncSearchResultsPlacement()})).observe(planSheet)}if(window.ResizeObserver&&activeNav){const hudObserver=new ResizeObserver(()=>requestAnimationFrame(syncNavigationHudGeometry));hudObserver.observe(activeNav);if($('navManeuverTop'))hudObserver.observe($('navManeuverTop'))}activeNav?.addEventListener('transitionend',()=>requestAnimationFrame(syncNavigationHudGeometry));planSheet?.addEventListener('scroll',()=>requestAnimationFrame(syncSearchResultsPlacement),{passive:true});requestAnimationFrame(syncFloatingLocate);window.addEventListener('resize',()=>{syncFloatingLocate();syncSearchResultsPlacement()},{passive:true});
function refreshResponsiveViewport(){
  clearTimeout(vanoViewportTimer);
  // Pinch zoom is an accessibility action, not a keyboard or orientation change.
  ...
  document.documentElement.style.setProperty('--vano-vh',`${Math.max(160,safeH)}px`);document.documentElement.style.setProperty('--vano-vw',`${Math.max(240,rawW)}px`);document.documentElement.style.setProperty('--vano-keyboard-bottom',`${Math.max(0,keyboardBottom)}px`);
