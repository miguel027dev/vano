...
const VANO_ROUTE_PURPLE='#7457F6',VANO_ROUTE_PURPLE_LIGHT='#B8ACFF',VANO_ROUTE_PURPLE_DARK='#5C42D7';
...
function setNavCameraState(next){if(!Object.values(NAV_CAMERA_STATES).includes(next))return;navCameraState=next;document.body?.setAttribute?.('data-nav-camera-state',next)}
...
function animatedRouteFlowGradient(phase=0,{base=VANO_ROUTE_PURPLE,accent=VANO_ROUTE_PURPLE_LIGHT,pulse='#F3FCFF',segments=7,pulseWidth=.085}={}){
...
function updateAnimatedRoutePaint(force=false){
  ...
  try{
    if(map.getLayer('route-selected-flow'))map.setPaintProperty('route-selected-flow','line-gradient',animatedRouteFlowGradient(planPhase,{base:VANO_ROUTE_PURPLE,accent:VANO_ROUTE_PURPLE_LIGHT,pulse:'#F8F6FF',segments:7,pulseWidth:.09}));
    if(map.getLayer('nav-route-flow'))map.setPaintProperty('nav-route-flow','line-gradient',animatedRouteFlowGradient(navPhase,{base:VANO_ROUTE_PURPLE,accent:VANO_ROUTE_PURPLE_DARK,pulse:'#FFFFFF',segments:8,pulseWidth:.072}));
...
async function installRuntimeLayers(){
  ...
  ensureLayer({id:'route-selected-glow',type:'line',source:'routes',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE_PURPLE,'line-width':['interpolate',['linear'],['zoom'],10,7.4,12,9.2,14,11.0,16,13.2,18,15.4,20,17.6],'line-opacity':.18,'line-blur':1.15}},routeBefore);
  ensureLayer({id:'route-selected',type:'line',source:'routes',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE_PURPLE,'line-width':['interpolate',['linear'],['zoom'],10,4.3,12,5.3,14,6.5,16,7.8,18,9.1,20,10.5],'line-opacity':1}},routeBefore);
  ensureLayer({id:'route-selected-flow',type:'line',source:'routes',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':animatedRouteFlowGradient(0,{base:VANO_ROUTE_PURPLE,accent:VANO_ROUTE_PURPLE_LIGHT,pulse:'#F8F6FF',segments:7,pulseWidth:.09}),'line-width':['interpolate',['linear'],['zoom'],10,1.4,12,1.8,14,2.2,16,2.7,18,3.2,20,3.7],'line-opacity':.95}},routeBefore);
  if(!map.getSource('nav-route'))map.addSource('nav-route',{type:'geojson',lineMetrics:true,data:emptyFC()});
  ensureLayer({id:'nav-route-glow',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE_PURPLE,'line-width':['interpolate',['linear'],['zoom'],10,8.0,12,9.8,14,11.8,16,14.0,18,16.2,20,18.4],'line-opacity':.10,'line-blur':1.0}},routeBefore);
  ...
  ensureLayer({id:'nav-route-core',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':['interpolate',['linear'],['line-progress'],0,VANO_ROUTE_PURPLE_LIGHT,1,VANO_ROUTE_PURPLE],'line-width':['interpolate',['linear'],['zoom'],10,4.5,12,5.5,14,6.6,16,7.9,18,9.3,20,10.8],'line-opacity':1}},routeBefore);
  ensureLayer({id:'nav-route-flow',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':animatedRouteFlowGradient(0,{base:VANO_ROUTE_PURPLE,accent:VANO_ROUTE_PURPLE_DARK,pulse:'#FFFFFF',segments:8,pulseWidth:.072}),'line-width':['interpolate',['linear'],['zoom'],10,1.5,12,1.9,14,2.3,16,2.8,18,3.3,20,3.8],'line-opacity':.94}},routeBefore);
  ensureGeoSource('nav-turn');
  ensureLayer({id:'nav-turn-glow',type:'line',source:'nav-turn',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE_PURPLE_LIGHT,'line-width':['interpolate',['linear'],['zoom'],12,7.5,16,10.5,20,13.5],'line-opacity':.18,'line-blur':1.2}},routeBefore);
...
function navRouteGradient(startAlong=0){
  const base=VANO_ROUTE_PURPLE,remaining=Math.max(1,routeTotalGeometry-(+startAlong||0)),segs=(isMotorizedProfile()?(selectedRoute?.traffic_segments||[]):[]).filter(x=>Number.isFinite(+x.distance_start_m)&&Number.isFinite(+x.distance_end_m)&&(+x.distance_end_m||0)>=startAlong);
  if(!segs.length||!routeTotalGeometry)return ['interpolate',['linear'],['line-progress'],0,VANO_ROUTE_PURPLE_LIGHT,.55,VANO_ROUTE_PURPLE,1,VANO_ROUTE_PURPLE_DARK];
...
function setNavigationFollow(enabled,recenter=true){followMode=!!enabled;setNavCameraState(followMode?(recenter?NAV_CAMERA_STATES.RECENTERING:NAV_CAMERA_STATES.FOLLOWING):NAV_CAMERA_STATES.FREE_LOOK);if(!followMode)stopCameraMotion();else if(recenter)markCameraIntent('follow',960);syncFollowButton();document.body.classList.toggle('nav-map-free',!followMode&&!!activeNav?.classList.contains('show'));if(followMode&&recenter&&lastNavPosition)scheduleCamera(lastNavPosition,true,nearestProgress(lastNavPosition))}
...
function calibrateNavigation(){
  ...
  const now=performance.now();followMode=true;mapFollowMode=true;navCameraMode='perspective';setNavCameraState(NAV_CAMERA_STATES.RECENTERING);markCameraIntent('recenter',1560);cameraCalibrationStartedAt=now;cameraCalibrationUntil=now+1180;navStartupBearing=null;navStartupBearingUntil=0;
