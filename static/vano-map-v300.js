...
function makeUserMarker(){
  document.querySelectorAll('.vano-user-puck-v230,.vano-user-puck-v300,.user-marker.vano-uploaded-user').forEach(node=>{if(node!==userMarkerElement){try{node.remove()}catch{}}});
  const el=document.createElement('div');el.className='vano-user-puck-v300';el.setAttribute('aria-hidden','true');el.style.visibility='visible';el.style.opacity='1';el.style.zIndex='99999';
  el.innerHTML=`<span class="vano-puck-accuracy"></span>
  <svg class="vano-puck-inline" viewBox="0 0 64 76" aria-hidden="true" focusable="false">
    <defs><filter id="vanoPuckShadow300" x="-40%" y="-35%" width="180%" height="190%"><feDropShadow dx="0" dy="3" stdDeviation="2.2" flood-color="#10151c" flood-opacity=".24"/></filter></defs>
    <g filter="url(#vanoPuckShadow300)">
      <path d="M32 4 56 62 35.3 52.7 32 71 28.7 52.7 8 62Z" fill="#fff" stroke="#fff" stroke-width="7" stroke-linejoin="round"/>
      <path d="M32 6.5 53.2 58.2 34.1 49.7 32 64.2 29.9 49.7 10.8 58.2Z" fill="#F47A45" stroke="#D96535" stroke-width="1.6" stroke-linejoin="round"/>
      <path d="M32 13.2 44.1 43.4 33.1 38.5 32 45.9 30.9 38.5 19.9 43.4Z" fill="#FFB18C" opacity=".34"/>
    </g>
  </svg>`;
...
function buildCameraTarget(ctx,ts){
  const p=ctx.p,m=cameraDynamicProgress(ctx,ts),speed=Math.max(0,+p.speed||0),step=currentStep(m.distanceAlong),turn=upcomingTurn(m,speed),junction=cameraJunctionContext(m,step,turn),signal=roadControlAhead('traffic_signal',m.distanceAlong,70),nearlyStopped=speed<.45,stoppedAtSignal=!!signal&&speed<.9,arrivalApproach=m.remaining<360&&m.progress>.82;
  const calibrating=ts<cameraCalibrationUntil,offRoute=Number.isFinite(+p?._rawOffRoute)?+p._rawOffRoute:0,bearingSpan=calibrating?72:Math.max(50,Math.min(110,58+speed*2.4)),routeBearing=routeBearingAtDistance(Math.max(0,m.distanceAlong-3),bearingSpan),gpsHeading=Number.isFinite(+p.heading)?+p.heading:null;
  ...
  if(startupLocked){const release=Math.max(0,Math.min(1,1-(navStartupBearingUntil-ts)/1800));rawTarget=blendBearing(navStartupBearing,rawTarget,release*.42)}
  else if(gpsHeading!=null&&speed>3&&(p.accuracy||0)<45&&bearingDelta(rawTarget,gpsHeading)<28)rawTarget=blendBearing(rawTarget,gpsHeading,.10);

  const vw=Math.max(320,window.innerWidth||320),vh=Math.max(320,window.innerHeight||480),landscape=vw>vh&&vh<=700,currentXY=routePointAtDistance(m.distanceAlong);let center=currentXY||[p.lon,p.lat],zoom,pitch,bearing,padding={top:0,bottom:0,left:0,right:0};
  let look=isMotorizedProfile()?Math.max(92,Math.min(235,104+speed*4.8)):34;
  if(junction.roundabout)look*=.66;else if(junction.large)look*=.72;else if(junction.decision||turn.angle>42)look*=.82;
  if(arrivalApproach)look*=.64;
  if(calibrating)look=NAV_CAMERA_HOME.lookAhead;
  const centerLead=navCameraMode==='top'?.28:(calibrating?NAV_CAMERA_HOME.centerLead:(junction.large?.54:.62));
  const target=routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*centerLead));setDriveCameraMood(speed,junction,turn);
  ...
  if(navCameraMode==='top'){
    ...
    const bottomRatio=calibrating?NAV_CAMERA_HOME.bottomRatio:(junction.large?.255:.285),topRatio=calibrating?NAV_CAMERA_HOME.topRatio:.045;
    padding=landscape?{top:Math.round(Math.min(42,Math.max(14,vh*.055))),bottom:Math.round(32+SAFE_AREA_BOTTOM),left:Math.round(Math.min(382,vw*.46)+22),right:72}:{top:Math.round(Math.min(62,Math.max(18,vh*topRatio))),bottom:Math.round(Math.min(310,Math.max(150,vh*bottomRatio))+SAFE_AREA_BOTTOM),left:Math.round(Math.min(30,Math.max(10,vw*.026))),right:Math.round(Math.min(26,Math.max(10,vw*.022)))};
  }
  if((p.accuracy||0)>70)zoom=Math.min(zoom,16.52);return{center,zoom,pitch,bearing,padding,rawTarget,stopped:nearlyStopped||stoppedAtSignal,junction,instant:!!ctx.instant,arrivalApproach,speed,nearlyStopped,previewing:!!previewing}
}
function primeNavigationCamera(p,m){
  if(!map||!p||!m)return;navCameraMode='perspective';navExperienceMode='immersive';const ts=performance.now(),locked=startupRouteBearing(m,p);markCameraIntent('launch',1080);if(Number.isFinite(locked)){navStartupBearing=locked;navStartupBearingUntil=ts+1800;lastCameraBearing=locked;lastRoutePuckBearing=locked}
  const target=buildCameraTarget({p:{...p},progressInfo:{...m},instant:true,updatedAt:ts},ts);if(Number.isFinite(navStartupBearing))target.bearing=navStartupBearing;lastCameraBearing=target.bearing;navLaunchAnimationUntil=ts+680;document.body.classList.add('nav-launching');
  try{const c=map.getCenter(),pad=map.getPadding?.()||{};cameraVisualState={center:[c.lng,c.lat],zoom:map.getZoom(),pitch:map.getPitch(),bearing:map.getBearing(),padding:{top:+pad.top||0,bottom:+pad.bottom||0,left:+pad.left||0,right:+pad.right||0}};map.stop?.();internalCameraMoveUntil=ts+780;map.easeTo({center:target.center,zoom:target.zoom,pitch:target.pitch,bearing:target.bearing,padding:target.padding,retainPadding:false,duration:640,essential:true,easing:t=>1-Math.pow(1-t,4)});setTimeout(()=>{document.body.classList.remove('nav-launching');cameraVisualState={center:[...target.center],zoom:target.zoom,pitch:target.pitch,bearing:target.bearing,padding:{...target.padding}}},640)}catch(e){console.debug('[VANO MAPS:camera-prime]',e)}updateMarkerHeading(Number.isFinite(navStartupBearing)?navStartupBearing:target.rawTarget)
}
...
function animatedRouteFlowGradient(phase=0,{base=VANO_ROUTE,accent=VANO_ROUTE_LIGHT,pulse='#FFF9F5',segments=5,pulseWidth=.05}={}){
...
function updateAnimatedRoutePaint(force=false){
  ...
  const now=performance.now();if(!force&&now-lastRouteFlowPaintAt<1200)return;lastRouteFlowPaintAt=now;
  // V300 deliberately keeps route paint stable. Motion belongs to the camera and HUD,
  // not to glowing/pulsing route stripes competing with the road geometry.
  try{
    if(map.getLayer('route-selected-flow'))map.setPaintProperty('route-selected-flow','line-gradient',['interpolate',['linear'],['line-progress'],0,VANO_ROUTE_LIGHT,.28,VANO_ROUTE,.88,VANO_ROUTE,1,VANO_ROUTE_DARK]);
    if(map.getLayer('route-selected-glow'))map.setPaintProperty('route-selected-glow','line-opacity',.07);
    if(map.getLayer('nav-route-flow'))map.setPaintProperty('nav-route-flow','line-opacity',0);
    if(map.getLayer('nav-route-glow'))map.setPaintProperty('nav-route-glow','line-opacity',.052);
    if(map.getLayer('alerts-marker-halo')){map.setPaintProperty('alerts-marker-halo','circle-opacity',.14);map.setPaintProperty('alerts-marker-halo','circle-radius',['interpolate',['linear'],['zoom'],10.5,11,16,18])}
  }catch(e){console.debug('[VANO MAPS:route-paint]',e)}
}
function routeFlowDelay(){return 1200}
function startRouteFlowAnimation(){updateAnimatedRoutePaint(true)}
...
async function installRuntimeLayers(){
  ...
  ensureLayer({id:'route-selected-glow',type:'line',source:'routes',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE,'line-width':['interpolate',['linear'],['zoom'],10,7.2,12,9.4,14,11.7,16,14.2,18,17.0,20,19.6],'line-opacity':.078,'line-blur':1.35}},routeBefore);
  ensureLayer({id:'route-selected',type:'line',source:'routes',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE,'line-width':['interpolate',['linear'],['zoom'],10,4.2,12,5.4,14,6.7,16,8.3,18,10.2,20,11.8],'line-opacity':1}},routeBefore);
  ensureLayer({id:'route-selected-flow',type:'line',source:'routes',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':animatedRouteFlowGradient(0,{base:VANO_ROUTE,accent:VANO_ROUTE_LIGHT,pulse:'#FFF9F5',segments:5,pulseWidth:.045}),'line-width':['interpolate',['linear'],['zoom'],10,.8,12,1.05,14,1.3,16,1.55,18,1.9,20,2.15],'line-opacity':.20}},routeBefore);
  if(!map.getSource('nav-route'))map.addSource('nav-route',{type:'geojson',lineMetrics:true,data:emptyFC()});
  ensureLayer({id:'nav-route-glow',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE,'line-width':['interpolate',['linear'],['zoom'],10,7.6,12,9.7,14,12.0,16,14.6,18,17.4,20,20.0],'line-opacity':.052,'line-blur':1.35}},routeBefore);
  ensureLayer({id:'nav-route-casing',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'rgba(255,255,255,.98)','line-width':['interpolate',['linear'],['zoom'],10,6.3,12,7.8,14,9.6,16,11.7,18,14.0,20,16.2],'line-opacity':.94}},routeBefore);
  ensureLayer({id:'nav-route-core',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':['interpolate',['linear'],['line-progress'],0,VANO_ROUTE_LIGHT,.20,VANO_ROUTE,.82,VANO_ROUTE,1,VANO_ROUTE_DARK],'line-width':['interpolate',['linear'],['zoom'],10,4.5,12,5.7,14,7.0,16,8.6,18,10.5,20,12.0],'line-opacity':1}},routeBefore);
  ensureLayer({id:'nav-route-flow',type:'line',source:'nav-route',layout:{'line-cap':'round','line-join':'round'},paint:{'line-gradient':animatedRouteFlowGradient(0,{base:VANO_ROUTE,accent:VANO_ROUTE_DARK,pulse:'#FFF7F2',segments:5,pulseWidth:.04}),'line-width':['interpolate',['linear'],['zoom'],10,.8,12,1.0,14,1.25,16,1.5,18,1.8,20,2.1],'line-opacity':.16}},routeBefore);
  ensureGeoSource('nav-turn');
  ensureLayer({id:'nav-turn-glow',type:'line',source:'nav-turn',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':VANO_ROUTE_LIGHT,'line-width':['interpolate',['linear'],['zoom'],12,10,16,16,20,22],'line-opacity':.18,'line-blur':1.2}},routeBefore);
...
function navRouteGradient(startAlong=0){
  ...
  if(!segs.length||!routeTotalGeometry)return ['interpolate',['linear'],['line-progress'],0,VANO_ROUTE_LIGHT,.25,VANO_ROUTE,.86,VANO_ROUTE,1,VANO_ROUTE_DARK];
...
function rerouteFrom(p){if(Date.now()-lastRerouteAt<25000)return;return performNavReroute(p,{manual:false})}
