...
function renderRouteVariants(){
  ...
  const list=(routes||[]).filter(r=>r?.geometry).slice().sort((a,b)=>(+a.duration||9e12)-(+b.duration||9e12)).slice(0,4),count=$('routeChooserCount');
  ...
  box.innerHTML=list.map(r=>{const active=routeClientKey(r)===activeKey,delta=Math.max(0,Math.round(((+r.duration||0)-fastEta)/60)),safeDelta=Math.round(routeSafetyScore(r)-fastSafety),exposure=routeExposurePct(r),traffic=Math.round(+r.traffic_score||0),reason=r.event_variant?'Evita movimento de evento':r.safety_variant?'Desvio verificado de segurança':r.adaptive_variant?'Corredor alternativo':r.micro_route?'Atalho de quarteirão':delta?`+${delta} min vs. mais rápida`:'Menor ETA';const gain=safeDelta>=4?`+${safeDelta} seg.`:safeDelta<=-4?`${safeDelta} seg.`:'';return `<button type="button" class="route-variant ${active?'active':''}" data-route-id="${esc(r.id)}"><small>${active?'Selecionada':variantLabel(r)}</small><b>${esc(fmtDuration(r.duration_min||(+r.duration||0)/60))}</b><span>${esc(fmtDistance(r.distance||0))}${gain?` · ${esc(gain)}`:''}</span><em>${esc(reason)}</em><div class="route-variant-signals"><i>Trânsito ${traffic||'—'}</i>${routeMode!=='fastest'?`<i>Exposição ${exposure.toFixed(exposure<10?1:0)}%</i>`:''}</div></button>`}).join('');
  box.querySelectorAll('[data-route-id]').forEach(btn=>btn.onclick=()=>{const found=routes.find(r=>String(r.id)===btn.dataset.routeId);if(!found)return;manualRouteSelection=true;selectedRoute=found;renderRoute();haptic(8);if(isMotorizedProfile())setTimeout(()=>nonFatal('event-manual-route',()=>checkEventDisruption(true,{pretrip:!activeNav?.classList.contains('show')})),180)})
}
