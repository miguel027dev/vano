...
function buildCameraTarget(ctx,ts){
  ...
  let centerLead=navCameraMode==='top'?.34:(calibrating?NAV_CAMERA_HOME.centerLead:(junction.roundabout?tune.roundLead:junction.large?tune.largeLead:junction.decision?tune.turnLead:tune.centerLead));if(motion.phase==='braking')centerLead*=profile==='motorcycle'?.88:.92;if(motion.phase==='crawl')centerLead*=.90;const target=routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*centerLead));setDriveCameraMood(speed,junction,turn);
