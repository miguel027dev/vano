...
const VANO_ROUTE='#F59A62', VANO_ROUTE_LIGHT='#FFC6A5', VANO_ROUTE_DARK=VANO_ROUTE;
...
const MAP_HOME_ZOOM=16.82; // V401: slightly closer default framing around the user.
...
const NAV_OFF_ROUTE_MIN_TRAVEL_M=26,NAV_OFF_ROUTE_MIN_TIME_MS=2200,NAV_OFF_ROUTE_MAX_ACCURACY_M=95;
...
function updateOffRouteTracker(raw,offRoute){
  ...
  offRouteClearHits=0;const point={lat:+raw.lat,lon:+raw.lon,accuracy:acc,speed:Number.isFinite(+raw.speed)?Math.max(0,+raw.speed):0,heading:Number.isFinite(+raw.heading)?+raw.heading:null};
...
function offRouteEligibleForReroute(){if(!offRouteStartedAt)return false;const elapsed=Date.now()-offRouteStartedAt,straight=offRouteStartRaw&&offRouteLastRaw?hav([offRouteStartRaw.lon,offRouteStartRaw.lat],[offRouteLastRaw.lon,offRouteLastRaw.lat]):0,acc=Math.max(1,+offRouteLastRaw?.accuracy||999),speed=Math.max(0,+offRouteLastRaw?.speed||0),strong=offRoutePeakM>=Math.max(58,offRouteThreshold(offRouteLastRaw)*1.35),excellent=acc<=26;const minTime=strong&&excellent?1350:excellent?NAV_OFF_ROUTE_MIN_TIME_MS:acc<=55?3000:4300,minTravel=strong&&excellent?16:excellent?NAV_OFF_ROUTE_MIN_TRAVEL_M:acc<=55?36:54;return elapsed>=minTime&&(offRouteTravelM>=minTravel||straight>=minTravel||offRoutePeakM>=Math.max(minTravel,offRouteThreshold(offRouteLastRaw)*1.12)||(speed>7&&strong&&elapsed>=1200))}
...
function requestFreshNavigationPosition(timeout=2600){return new Promise((ok,no)=>{if(!navigator.geolocation)return no({code:0});navigator.geolocation.getCurrentPosition(ok,no,{enableHighAccuracy:true,timeout:Math.max(1200,timeout),maximumAge:0})})}
...
function remainingNavGeometry(startAlong=0){
  ...
  // Keep only a tiny tail under the puck. This makes the orange path disappear
  // continuously behind the user instead of vanishing in large visual chunks.
  const measured=Math.max(0,+startAlong||0),visual=Number.isFinite(puckRouteAlong)?Math.max(0,+puckRouteAlong):measured,anchor=Math.min(measured,visual),speed=Math.max(0,+puckRouteTargetSpeed||+lastNavPosition?.speed||0),trail=Math.max(2.4,Math.min(5.5,2.4+speed*.12));
  const start=Math.max(0,Math.min(routeTotalGeometry-0.01,anchor-trail)),pt=routePointAtDistance(start);let lo=0,hi=routeCumulative.length-1;while(lo<hi){const mid=(lo+hi)>>1;if(routeCumulative[mid]<start)lo=mid+1;else hi=mid}const coords=[pt,...c.slice(Math.max(1,lo))].filter(Boolean),clean=[];for(const x of coords){const prev=clean.at(-1);if(!prev||hav(prev,x)>.25)clean.push(x)}return clean.length>1?{type:'LineString',coordinates:clean}:selectedRoute.geometry;
}
...
function buildCameraTarget(ctx,ts){
  ...
  const visualPuck=(puckDisplayPos&&Number.isFinite(+puckDisplayPos.lon)&&Number.isFinite(+puckDisplayPos.lat))?puckDisplayPos:p;let zoom,pitch,bearing,center=[+visualPuck.lon,+visualPuck.lat],padding={top:0,bottom:0,left:0,right:0};
  ...
  let look=calibrating?NAV_CAMERA_HOME.lookAhead:cameraLookAhead(speed,m,motion);if(!calibrating){if(junction.decision)look*=junction.large?.88:.94;if(junction.roundabout)look*=profile==='motorcycle'?.82:.76;if(arrivalApproach)look*=.68;look*=profile==='motorcycle'?.94:.88;if(urban.close&&!junction.large)look*=1-Math.min(.16,urban.density*.14);look=Math.max(tune.lookMin,Math.min(profile==='motorcycle'?285:isMotorizedProfile()?220:54,look));if(startupRatio>.01)look*=.82+.18*(1-startupRatio)}
  let centerLead=calibrating?NAV_CAMERA_HOME.centerLead:(junction.roundabout?tune.roundLead:junction.large?tune.largeLead:junction.decision?tune.turnLead:tune.centerLead);if(motion.phase==='braking')centerLead*=profile==='motorcycle'?.88:.92;if(motion.phase==='crawl')centerLead*=.90;if(urban.close&&!junction.decision)centerLead*=1-Math.min(.07,urban.density*.06);const target=routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*centerLead));setDriveCameraMood(speed,junction,turn);
  {
    // The route decides bearing/zoom/context; the user's puck decides the camera anchor.
    // Keeping center on the puck prevents the camera from visually chasing the route line after a reroute.
    if(target&&previewing)center=target;
  ...
  if(gpsWeak){
    zoom=Math.min(zoom,(profile==='motorcycle'?16.50:16.60)+startupBoost*.14);
    pitch=Math.max(38,pitch-(gpsPoor?7:4));
...
function cameraMotionProfile(target,ts,dt){
  ...
  const speed=Math.max(0,+target?.speed||0),intent=activeCameraIntent(ts),largeTurn=!!target?.junction?.large,turning=!!target?.junction?.decision,roundabout=!!target?.junction?.roundabout,arriving=!!target?.arrivalApproach,stopped=!!target?.stopped,instant=!!target?.instant&&ts-(cameraTargetContext?.updatedAt||ts)<420,preview=!!target?.previewing||intent==='preview',phase=target?.motion?.phase||'steady',rerouting=navCameraState===NAV_CAMERA_STATES.REROUTING;
  ...
  if(rerouting||intent==='reroute')return moto?{label:'rerouting-moto',posTau:68,viewTau:92,bearingTau:118,centerEps:.055,zoomEps:.0028,bearingEps:.12}:{label:'rerouting',posTau:82,viewTau:108,bearingTau:145,centerEps:.06,zoomEps:.003,bearingEps:.14};
  if(target?.gpsPoor)return moto?{label:'gps-poor-moto',posTau:190,viewTau:235,bearingTau:920,centerEps:.11,zoomEps:.005,bearingEps:.28}:{label:'gps-poor',posTau:235,viewTau:285,bearingTau:1050,centerEps:.13,zoomEps:.0055,bearingEps:.32};
  if(target?.gpsWeak)return moto?{label:'gps-weak-moto',posTau:155,viewTau:195,bearingTau:540,centerEps:.09,zoomEps:.0045,bearingEps:.21}:{label:'gps-weak',posTau:195,viewTau:235,bearingTau:650,centerEps:.105,zoomEps:.0048,bearingEps:.25};
  ...
  if(target?.urbanClose&&!target?.junction?.decision)return moto?{label:'urban-focus-moto',posTau:112,viewTau:205,bearingTau:185,centerEps:.068,zoomEps:.0031,bearingEps:.16}:{label:'urban-focus',posTau:142,viewTau:245,bearingTau:220,centerEps:.078,zoomEps:.0034,bearingEps:.19};
...
async function installRuntimeLayers(){
  ...
  ensureLayer({id:'road-controls-pulse',type:'circle',source:'road-controls',minzoom:11.8,layout:{'circle-pitch-alignment':'viewport'},paint:{'circle-radius':['case',['==',['get','type'],'speed_camera'],['interpolate',['linear'],['zoom'],11.8,5.5,15.5,8,18,9],['interpolate',['linear'],['zoom'],11.8,0,13.79,0,13.8,8,18,8]],'circle-color':['match',['get','type'],'speed_camera','#ef4444','traffic_signal','#ffb13b','stop_sign','#ff6166','yield_sign','#ff8b43','traffic_sign','#6db9ff','#ff9a45'],'circle-opacity':['case',['==',['get','type'],'speed_camera'],.15,['interpolate',['linear'],['zoom'],13.79,0,13.8,.13]],'circle-blur':.58}});
  ensureLayer({id:'road-controls-dot',type:'symbol',source:'road-controls',minzoom:11.8,layout:{'icon-image':roadControlIconExpr(),'icon-size':['case',['==',['get','type'],'speed_camera'],['interpolate',['linear'],['zoom'],11.8,.36,13.4,.48,15.5,.60,18,.74],['interpolate',['linear'],['zoom'],11.8,0,13.79,0,13.8,.48,15.5,.60,18,.74]],'icon-allow-overlap':true,'icon-ignore-placement':true,'symbol-sort-key':['match',['get','type'],'speed_camera',20,'traffic_signal',10,5],'icon-pitch-alignment':'viewport','icon-rotation-alignment':'viewport'}});
...
function startSignalPulse(){
  // V401: paint expressions are defined by installRuntimeLayers. Do not replace
  // them here, otherwise low-zoom radar-only visibility gets overwritten.
...
function syncImmersiveButton(){navCameraMode='perspective';navExperienceMode='immersive';document.body.classList.toggle('nav-immersive',!!activeNav?.classList.contains('show'))}
function setNavigationExperience(mode,{recenter=true,announce=false}={}){
  // V303: one adaptive perspective camera. This remains as a compatibility shim
  // for older/native callers, but there is no longer a user-facing 2D/3D state.
  navExperienceMode='immersive';navCameraMode='perspective';followMode=true;markCameraIntent('recenter',900);syncFollowButton();syncImmersiveButton();if(recenter&&lastNavPosition)scheduleCamera(lastNavPosition,true,nearestProgress(lastNavPosition));if(announce)showToast('Câmera de navegação recalibrada.')
}
...
function syncNavCameraButton(){navCameraMode='perspective';navExperienceMode='immersive';document.body.classList.toggle('nav-immersive',!!activeNav?.classList.contains('show'))}
...
map.on('move',()=>{if(!internalCameraMoveActive())scheduleTrafficSnapshot(false)});map.on('zoomend',()=>syncAlertDomMarkers());map.on('moveend',()=>{if(internalCameraMoveActive()){syncAlertDomMarkers(lastAlertRecords);scheduleSignalRefresh(false);return}refreshAlerts(false);scheduleSignalRefresh(false);refreshNearbyDrivers(false);scheduleTrafficSnapshot(false)});map.on('sourcedata',e=>{if(e.sourceId==='vano-mapbox-traffic'&&e.isSourceLoaded)scheduleTrafficSnapshot(false)});map.on('click','road-controls-dot',e=>{const f=e.features?.[0];if(!f)return;const p=f.properties||{},src=String(p.source||'').replaceAll('_',' '),speed=String(p.maxspeed||'').trim(),official=String(p.official)==='true'||p.official===true;new mapboxgl.Popup({closeButton:false,offset:12}).setLngLat(f.geometry.coordinates).setHTML(`<b>${esc(p.label||'Sinalização viária')}</b>${speed?`<div style="font-size:11px;margin-top:4px">Limite informado: <b>${esc(speed)} km/h</b></div>`:''}<div style="color:#9299a5;font-size:9px;margin-top:4px">${official?'Fonte pública oficial':'Base cartográfica/comunitária'}${src?' · '+esc(src):''} · confirme a sinalização real da via</div>`).addTo(map)});map.on('click','vano-traffic-hotspot-dot',e=>{const f=e.features?.[0];if(!f)return;const severe=f.properties?.bucket==='severe';new mapboxgl.Popup({closeButton:false,offset:12}).setLngLat(f.geometry.coordinates).setHTML(`<b style="color:${severe?'#ff6254':'#ff9b43'}">${esc(f.properties?.label||'Trânsito intenso')}</b><div style="color:#9299a5;font-size:9px;margin-top:4px">Fluxo viário detectado nesta área · raio do mapa: 3 km</div>`).addTo(map)});
...
function radarRadiusForCamera(){
  if(!map)return 0;const z=map.getZoom();if(z<11.8)return 0;
  const c=map.getCenter?.(),b=map.getBounds?.();if(!c||!b)return z<13.2?6500:z<14.4?5200:3200;
  const corners=[[b.getWest(),b.getSouth()],[b.getWest(),b.getNorth()],[b.getEast(),b.getSouth()],[b.getEast(),b.getNorth()]];
  const edge=Math.max(...corners.map(pt=>hav([c.lng,c.lat],pt)));
  // Cover the entire visible viewport plus a small prefetch belt so radar icons
  // are already present as the camera approaches them. The backend caps at 6.5 km.
  return Math.max(1600,Math.min(6500,Math.ceil((edge*1.22+350)/100)*100));
}
...
function updateRoadLayer(extra=[]){
  ...
  const signalRadius=signalRadiusForCamera(),radarRadius=radarRadiusForCamera(),center=map.getCenter?.(),bounds=map.getBounds?.();
  if((!signalRadius&&!radarRadius)||!center||!bounds){map.getSource('road-controls').setData(emptyFC());return}
  ...
  const visible=dedupeRoadItems([...base,...roadAwareness,...extra]).filter(x=>{
    const type=String(x.type||'');if(!types.has(type)||!Number.isFinite(+x.lat)||!Number.isFinite(+x.lon))return false;
    const lon=+x.lon,lat=+x.lat,allowedRadius=type==='speed_camera'?radarRadius:signalRadius;if(!allowedRadius)return false;
    // Camera viewport drives visibility. A radar does not require the user or the
    // active route to pass over it; panning the map near it is enough.
    if(!(lon>=bounds.getWest()&&lon<=bounds.getEast()&&lat>=bounds.getSouth()&&lat<=bounds.getNorth()&&hav([center.lng,center.lat],[lon,lat])<=allowedRadius))return false;
    if(type!=='speed_camera'&&activeNav?.classList.contains('show')&&selectedRoute){const m=nearestProgressOnRoute({lat,lon},selectedRoute);if(m.offRoute>85)return false}
    return true;
  });
  const byDistance=(a,b)=>hav([center.lng,center.lat],[+a.lon,+a.lat])-hav([center.lng,center.lat],[+b.lon,+b.lat]);
  const radars=visible.filter(x=>String(x.type)==='speed_camera').sort(byDistance).slice(0,28);
  ...
  map.getSource('road-controls').setData({type:'FeatureCollection',features:shown.map(x=>({type:'Feature',properties:{type:x.type,label:x.label||roadLabelIcon(x.type),source:x.source||'',official:!!x.official,maxspeed:x.maxspeed||x.speed_limit||'',radar_type:x.radar_type||''},geometry:{type:'Point',coordinates:[+x.lon,+x.lat]}}))});
}
...
function renderRoadAwareness(p,m){if(!selectedRoute)return;const types=roadControlTypes(),all=dedupeRoadItems([...(selectedRoute.road_controls||[]),...roadAwareness]).filter(x=>types.has(String(x.type||''))),ahead=[];for(const x of all){const n=nearestProgress({lat:+x.lat,lon:+x.lon}),delta=n.distanceAlong-m.distanceAlong;if(n.offRoute<=105&&delta>=-12&&delta<=1800)ahead.push({...x,ahead:Math.max(0,delta),off:n.offRoute})}ahead.sort((a,b)=>a.ahead-b.ahead||a.off-b.off);const next=ahead[0];$('roadAhead').textContent=next?roadLabelIcon(next.type):'Nenhuma sinalização mapeada logo à frente';$('roadAheadDistance').textContent=next?`${fmtDistance(next.ahead)} à frente · ${next.official?'fonte oficial':(next.source==='openstreetmap'?'OpenStreetMap':'base VANO')}`:'Continuamos verificando o corredor da rota';const lim=currentSpeedLimit(m.index),sign=$('speedSign');if(lim?.speed!=null){currentMappedSpeedLimitKmh=Math.round(+lim.speed);sign.classList.remove('unknown');sign.textContent=currentMappedSpeedLimitKmh;sign.title=`Limite ${lim.speed} ${lim.unit||'km/h'}`}else{currentMappedSpeedLimitKmh=null;sign.classList.add('unknown');sign.textContent='—';sign.title='Limite não mapeado'}updateFloatingSpeedometer(lastDisplayedSpeedKmh);const radar=upcomingSpeedCamera(m.distanceAlong,1400);if(radar)maybeWarnSpeedCamera(radar)}
async function refreshVisibleSignals(force=false){
  if(!mapPrefs.liveSignals)return;const native=!!window.__vanoNativeMapActive&&!!activeNav?.classList.contains('show');if(!native&&!map?.loaded())return;
  const now=Date.now(),signalRadius=native?2200:signalRadiusForCamera(),radarRadius=native?2600:radarRadiusForCamera(),radius=Math.max(signalRadius,radarRadius);
  if(!radius){roadAwareness=[];updateRoadLayer();return}
  const navPos=native?(lastNavPosition||userLocation):null,c=navPos?{lat:+navPos.lat,lng:+navPos.lon}:map.getCenter(),prec=3,key=`${c.lat.toFixed(prec)}:${c.lng.toFixed(prec)}:${radius}`;
  // Only suppress an actual duplicate viewport. A newly panned camera must fetch
  // immediately even if the previous request happened a second ago.
  if(!force&&key===lastSignalViewportKey&&now-lastSignalFetchAt<12000){updateRoadLayer();return}
  lastSignalViewportKey=key;lastSignalFetchAt=now;signalController?.abort();signalController=new AbortController();
  try{const q=new URLSearchParams({lat:c.lat,lon:c.lng,radius,controls_only:'1',viewport:'1'}),r=await fetch('/api/road-awareness?'+q,{signal:signalController.signal,cache:'no-store'}),d=await r.json();if(r.ok){const types=roadControlTypes();roadAwareness=(d.items||[]).filter(x=>types.has(String(x.type||''))).slice(0,180);updateRoadLayer()}}catch(e){if(e?.name!=='AbortError')console.warn('[VANO MAPS:signals]',e)}
}
...
function routeOverviewPadding(){
  const w=Math.max(320,window.innerWidth||320),h=Math.max(480,window.innerHeight||480);
  if(w>=1500)return{top:88,bottom:88,left:565,right:110};
  if(w>=1180)return{top:82,bottom:82,left:535,right:90};
  if(w>=760)return{top:76,bottom:76,left:490,right:64};
  return{top:Math.min(220,Math.max(150,h*.22)),bottom:Math.min(325,Math.max(245,h*.36)),left:35,right:35}
}
function animateRouteChoice(el){
  if(!el||REDUCED_MOTION||typeof el.animate!=='function')return;
  try{el.animate([{transform:'scale(.965)'},{transform:'scale(1.018)'},{transform:'scale(1)'}],{duration:260,easing:'cubic-bezier(.2,.8,.2,1)'})}catch{}
}
function drawRoute(){if(!selectedRoute?.geometry||!map.getSource('routes'))return;if(activeNav?.classList.contains('show')){buildMetrics();setNavigationRouteFocus(true);setNavRouteData();updateAnimatedRoutePaint(true);return}setNavigationRouteFocus(false);map.getSource('routes').setData({type:'FeatureCollection',features:[{type:'Feature',properties:{},geometry:selectedRoute.geometry}]});map.getSource('route-alternatives')?.setData(alternativeRoutesFC());map.getSource('route-progress')?.setData(emptyFC());updateTrafficSegmentLayer();updateAnimatedRoutePaint(true);const b=new mapboxgl.LngLatBounds();selectedRoute.geometry.coordinates.forEach(c=>b.extend(c));map.stop?.();map.fitBounds(b,{padding:routeOverviewPadding(),maxZoom:16.25,duration:REDUCED_MOTION?0:(window.innerWidth>=760?680:780),essential:true,easing:t=>t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2})}
...
function toggleNavigationCamera(){return calibrateNavigation()}
...
function updateNavigation(g,simulated=false){
  ...
  const routeEraseGap=performanceTier==='eco'?420:performanceTier==='normal'?265:175,routeEraseStep=performanceTier==='eco'?2.0:performanceTier==='normal'?1.2:.75;if(Math.abs(navLastAlong-navLastPaintAlong)>routeEraseStep||now-lastNavRoutePaintAt>routeEraseGap){lastNavRoutePaintAt=now;navLastPaintAlong=navLastAlong;setNavRouteData(navLastAlong)}if(now-lastProgressPaintAt>280){lastProgressPaintAt=now;updateTurnHighlight(m.distanceAlong,step)}
