...
function buildCameraTarget(ctx,ts){
  ...
  return{center,zoom,pitch,bearing,padding,rawTarget,stopped:nearlyStopped||stoppedAtSignal,junction,instant:!!ctx.instant,arrivalApproach,speed,nearlyStopped,previewing:!!previewing,motion,gpsWeak,gpsPoor}
}
...
[originInput,destinationInput].forEach(inp=>{
  ...
  inp.addEventListener('focus',()=>{activeSearchKind=kind;setSearchInteraction(true);planSheet?.classList.add('planner-search-active');if(kind==='destination'&&window.innerWidth<900&&welcomeState?.style.display!=='none'){planSheet?.classList.remove('sheet-collapsed');planSheet?.classList.add('sheet-expanded');const grab=$('planGrab');grab?.setAttribute('aria-expanded','true');grab?.setAttribute('aria-label','Recolher painel de destino');if(grab)grab.dataset.state='expanded';try{planSheet.scrollTo({top:0,behavior:'auto'})}catch{if(planSheet)planSheet.scrollTop=0}}lockIOSInputViewport(inp);refreshResponsiveViewport();requestAnimationFrame(()=>{lockIOSInputViewport(inp);syncFloatingLocate();syncSearchResultsPlacement()});if(inp.value.trim().length>=SEARCH_FAST_MIN&&!searchComposing)queueSearch(inp,kind);else{hideResults();if(kind==='destination')showSavedSearchSuggestions(kind)}});
