...
function syncManeuverCardDensity(step){const card=$('navManeuverTop');if(!card)return;const instruction=maneuverLabel(step),street=stepStreet(step),score=instruction.length+Math.min(38,street.length*.55);card.dataset.density=score>58?'tight':score>42?'compact':'normal';card.dataset.maneuverType=String(step?.type||'continue').toLowerCase()}
