...
function cameraLookAhead(speed,m){let base=isMotorizedProfile()?Math.max(112,Math.min(330,118+speed*6.1)):34;const turn=upcomingTurn(m,speed);if(turn.angle>58)base*=.62;else if(turn.angle>34)base*=.76;const step=currentStep(m?.distanceAlong||0);if((step.remainingInStep??9999)<120)base*=.78;return Math.max(isMotorizedProfile()?74:28,base)}
...
function buildCameraTarget(ctx,ts){
  ...
  const offRoute=Number.isFinite(+p?._rawOffRoute)?+p._rawOffRoute:0,bearingSpan=calibrating?72:Math.max(42,Math.min(125,52+speed*3.2)),routeBearing=routeBearingAtDistance(Math.max(0,m.distanceAlong-(calibrating?2:6)),bearingSpan),gpsHeading=Number.isFinite(+p.heading)?+p.heading:null;
  ...
  if(navCameraMode==='top'){
    if(target&&speed>3)center=routePointAtDistance(Math.min(routeTotalGeometry-1,m.distanceAlong+look*.24))||center;
    zoom=Math.max(16.05,Math.min(17.05,16.86-Math.min(.48,speed*.018)));pitch=0;bearing=0;padding=landscape?{top:18,bottom:20+SAFE_AREA_BOTTOM,left:Math.round(Math.min(382,vw*.46)+22),right:68}:{top:26,bottom:72+SAFE_AREA_BOTTOM,left:18,right:18};
  }else{
    if(target)center=target;
    const speedOpen=Math.min(.48,speed*.017),turnOpen=junction.roundabout?.44:junction.large?.34:(junction.decision?.16:0),arrivalClose=arrivalApproach?.14:0;
    zoom=calibrating?NAV_CAMERA_HOME.zoom:17.24-speedOpen-turnOpen+arrivalClose;
    zoom=Math.max(16.35,Math.min(17.28,zoom));
    pitch=calibrating?NAV_CAMERA_HOME.pitch:(junction.roundabout?41:junction.large?44:(junction.decision||turn.angle>48?47:52));
    if(arrivalApproach)pitch=Math.min(pitch,46);
...
function cameraMotionProfile(target,ts,dt){
  ...
  if(calibrating){const t=Math.max(0,Math.min(1,(ts-cameraCalibrationStartedAt)/1040)),e=t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;return{label:'calibrating',posTau:Math.max(88,255-150*e),viewTau:Math.max(100,300-170*e),bearingTau:Math.max(118,350-185*e),centerEps:.07,zoomEps:.0035,bearingEps:.16}}
